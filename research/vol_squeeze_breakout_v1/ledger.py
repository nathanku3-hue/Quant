"""Hash-chained append-only VSB M0 prediction tape."""
...
from research.vol_squeeze_breakout_v1.runner import verify_prediction_batch


LEDGER_LINK_SCHEMA = "vol_squeeze_breakout_prediction_ledger_link_v1"
...
def append_prediction_batch(
    ...
    *,
    ...
    """Append one sealed decision-date batch under exclusive fail-closed custody."""
    ...
    try:
        try:
            ...
            raise FileExistsError("vsb_prediction_ledger_writer_lock_exists") from exc
        ...
        if any(str(entry["prediction_batch_sha256"]) == batch_sha for entry in existing):
            raise FileExistsError("vsb_prediction_batch_already_in_ledger")
        if any(str(entry["decision_session_date"]) == decision_date for entry in existing):
            raise FileExistsError("vsb_prediction_decision_date_already_in_ledger")
        ...
        if recorded < made_at:
            raise ValueError("vsb_prediction_ledger_recorded_before_prediction")
        if existing:
            previous_recorded = _timestamp(existing[-1]["recorded_at"], "previous_recorded_at")
            if recorded < previous_recorded:
                raise ValueError("vsb_prediction_ledger_recorded_at_not_monotonic")
        ...
        chain_hash = domain_hash(
            "VOL_SQUEEZE_BREAKOUT_V1:PREDICTION_LEDGER_LINK",
        ...
        try:
            if os.write(fd, line) != len(line):
                raise OSError("vsb_prediction_ledger_partial_append")
            os.fsync(fd)
        finally:
            os.close(fd)
        ...
        if not verified or verified[-1] != entry:
            raise ValueError("vsb_prediction_ledger_post_append_verification_failed")
        return entry
    finally:
...
def load_prediction_tape(ledger_path: str | Path) -> list[dict[str, Any]]:
    ...
    if not raw.endswith(b"\n"):
        raise ValueError("vsb_prediction_ledger_partial_final_line")
    entries: list[dict[str, Any]] = []
    for line_number, raw_line in enumerate(raw.splitlines(), start=1):
        if not raw_line:
            raise ValueError(f"vsb_prediction_ledger_blank_line:{line_number}")
        try:
            parsed = json.loads(raw_line.decode("utf-8"))
        except (UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise ValueError(f"vsb_prediction_ledger_invalid_json:{line_number}") from exc
        if not isinstance(parsed, dict):
            raise ValueError(f"vsb_prediction_ledger_entry_mapping_required:{line_number}")
...
def verify_prediction_tape(entries: Sequence[Mapping[str, Any]]) -> None:
    ...
    for sequence, raw in enumerate(entries):
        if not isinstance(raw, Mapping):
            raise ValueError("vsb_prediction_ledger_entry_mapping_required")
        entry = dict(raw)
        ...
        if chain_hash != expected:
            raise ValueError("vsb_prediction_ledger_chain_hash_mismatch")
        if entry.get("schema_version") != LEDGER_LINK_SCHEMA:
            raise ValueError("vsb_prediction_ledger_schema_invalid")
        if int(entry.get("sequence", -1)) != sequence:
            raise ValueError("vsb_prediction_ledger_sequence_invalid")
        if entry.get("previous_chain_hash") != previous_hash:
            raise ValueError("vsb_prediction_ledger_previous_hash_invalid")
        if entry.get("family_id") != FAMILY_ID or entry.get("implementation_id") != IMPLEMENTATION_ID:
            raise ValueError("vsb_prediction_ledger_identity_invalid")
        if entry.get("prediction_ledger_scope") != PREDICTION_LEDGER_SCOPE:
            raise ValueError("vsb_prediction_ledger_scope_invalid")
        if entry.get("evaluation_status") != "UNMATURED_NOT_EVALUATED":
            raise ValueError("vsb_prediction_ledger_evaluation_status_invalid")

        batch = entry.get("batch")
        if not isinstance(batch, Mapping):
            raise ValueError("vsb_prediction_ledger_batch_mapping_required")
        verify_prediction_batch(batch)
        batch_sha = str(batch["prediction_batch_sha256"])
        decision_date = str(batch["decision_session_date"])
        prediction_ids = [str(row["prediction_id"]) for row in batch["rows"]]
        if batch_sha != str(entry.get("prediction_batch_sha256")):
            raise ValueError("vsb_prediction_ledger_batch_sha_mismatch")
        if prediction_ids != list(entry.get("prediction_ids") or []):
            raise ValueError("vsb_prediction_ledger_prediction_ids_mismatch")
        if decision_date != str(entry.get("decision_session_date")):
            raise ValueError("vsb_prediction_ledger_decision_date_mismatch")
        if str(batch["decision_context_id"]) != str(entry.get("decision_context_id")):
            raise ValueError("vsb_prediction_ledger_decision_context_mismatch")
        if str(batch["knowledge_cutoff"]) != str(entry.get("knowledge_cutoff")):
            raise ValueError("vsb_prediction_ledger_knowledge_cutoff_mismatch")
        if str(batch["prediction_made_at"]) != str(entry.get("prediction_made_at")):
            raise ValueError("vsb_prediction_ledger_prediction_time_mismatch")
        if int(batch["risk_set_count"]) != int(entry.get("risk_set_count", -1)):
            raise ValueError("vsb_prediction_ledger_risk_set_count_mismatch")
        if int(batch["support_count"]) != int(entry.get("support_count", -1)):
            raise ValueError("vsb_prediction_ledger_support_count_mismatch")
        if str(batch["support_breadth"]) != str(entry.get("support_breadth")):
            raise ValueError("vsb_prediction_ledger_support_breadth_mismatch")

        if batch_sha in batch_hashes:
            raise ValueError("vsb_prediction_ledger_duplicate_batch")
        if decision_date in decision_dates:
            raise ValueError("vsb_prediction_ledger_duplicate_decision_date")
        if any(prediction_id in prediction_ids_seen for prediction_id in prediction_ids):
            raise ValueError("vsb_prediction_ledger_duplicate_prediction_id")
