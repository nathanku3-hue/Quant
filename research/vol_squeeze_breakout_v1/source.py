"""Prospective source-bound producer for ``VOL_SQUEEZE_BREAKOUT_v1``.

This module performs no provider/network acquisition. It accepts already-landed,
hash-receipted same-day broad-risk-set rows plus daily market history, validates
the preregistered VSB universe law, and emits the exact Alpha-PIT risk-set and
VSB market-history artifacts consumed by the provider-blind M0 core.
"""
...
RISK_SET_SOURCE_AUTHORITY_SCHEMA = "vsb_us_primary_common_source_authority_v1"
RISK_SET_SOURCE_ROW_SCHEMA = "vsb_us_primary_common_source_row_v1"
MARKET_SOURCE_ROW_SCHEMA = "vsb_primary_market_source_row_v1"
SOURCE_PRODUCTION_SCHEMA = "vol_squeeze_breakout_source_production_v1"
PROSPECTIVE_CAPTURE_MODE = "PROSPECTIVE_SAME_DAY_SNAPSHOT"
...
class VSBSourceBoundBackend(AlphaPITBackendV1):
    """One-object backend exposing only the already-sealed VSB risk set."""
    ...
    def risk_set(self, *, as_of: datetime, research_mode: ResearchMode) -> ArtifactRef:
        ...
        if self._risk_set.manifest.get("as_of") != expected_as_of:
            raise ValueError("vsb_source_backend_as_of_mismatch")
        if self._risk_set.manifest.get("research_mode") != ResearchMode(research_mode).value:
            raise ValueError("vsb_source_backend_mode_mismatch")
        return self._risk_set

    def observations(self, **_: Any) -> ArtifactRef:
        raise ValueError("vsb_source_backend_snapshot_observations_forbidden")

    def source_claims(self, **_: Any) -> ArtifactRef:
        raise ValueError("vsb_source_backend_claims_forbidden")

    def expectations(self, **_: Any) -> ArtifactRef:
        raise ValueError("vsb_source_backend_expectations_forbidden")

    def outcomes(self, **_: Any) -> ArtifactRef:
        raise ValueError("vsb_source_backend_outcomes_forbidden")


def build_vsb_source_production(
    *,
    as_of: datetime,
    ...
    """Close source bytes into the VSB input packet without outcome access."""

    mode = ResearchMode(research_mode)
    if mode is ResearchMode.DISCOVERY:
        raise ValueError("vsb_source_discovery_mode_forbidden")
    ...
    risk_authority_sha = domain_hash(
        "VOL_SQUEEZE_BREAKOUT_V1:RISK_SET_SOURCE_AUTHORITY",
        canonical_value(authority),
    )
    risk_set_id = domain_hash(
        "VOL_SQUEEZE_BREAKOUT_V1:RISK_SET",
        canonical_value(
            {
                "family_id": FAMILY_ID,
    ...
    market_history = build_artifact_ref(
        ...
        request={
            ...
            "source_authority_sha256": domain_hash(
                "VOL_SQUEEZE_BREAKOUT_V1:MARKET_SOURCE_AUTHORITY",
                canonical_value(
                    {
    ...
    api = open_alpha_pit_session(
        ...
        backend=VSBSourceBoundBackend(risk_set),
        family_contract=FAMILY_DATA_CONTRACT,
    )
    input_packet = build_vsb_input_packet(
        api=api,
    ...
    production_body = {
        ...
        "aov_universe_reuse": False,
        "financial_alpha_evidence": 0,
    }
    return VSBSourceProduction(
        risk_set=risk_set,
        market_history=market_history,
        input_packet=input_packet,
        source_production_sha256=domain_hash(
            "VOL_SQUEEZE_BREAKOUT_V1:SOURCE_PRODUCTION",
...
def _validate_risk_set_authority(
    ...
    *,
    ...
    if not isinstance(raw, Mapping) or set(raw) != RISK_SET_SOURCE_AUTHORITY_FIELDS:
        raise ValueError("vsb_risk_set_source_authority_fields_invalid")
    authority = dict(raw)
    if authority.get("schema_version") != RISK_SET_SOURCE_AUTHORITY_SCHEMA:
        raise ValueError("vsb_risk_set_source_authority_schema_invalid")
    if authority.get("family_id") != FAMILY_ID:
        raise ValueError("vsb_risk_set_source_authority_family_invalid")
    if authority.get("risk_set_spec_id") != RISK_SET_SPEC_ID:
        raise ValueError("vsb_risk_set_source_authority_spec_invalid")
    if authority.get("capture_mode") != PROSPECTIVE_CAPTURE_MODE:
        raise ValueError("vsb_risk_set_source_capture_mode_invalid")
    if _date_value(authority.get("decision_date"), field="source_authority_decision_date") != decision_date:
        raise ValueError("vsb_risk_set_source_decision_date_mismatch")
    for field in (
        "growth_filter_applied",
        "rule100_filter_applied",
        "current_survivor_back_projection_used",
        ...
        if authority.get(field) is not False:
            raise ValueError("vsb_risk_set_source_forbidden_flag:" + field)
    ...
    if not isinstance(bound_hashes, list) or set(map(str, bound_hashes)) != receipt_hashes:
        raise ValueError("vsb_risk_set_source_receipts_not_exact")
...
def _validate_risk_set_source_rows(
    ...
    *,
    ...
    if not rows:
        raise ValueError("vsb_risk_set_source_rows_required")
    ...
    for raw in rows:
        if not isinstance(raw, Mapping) or set(raw) != RISK_SET_SOURCE_ROW_FIELDS:
            raise ValueError("vsb_risk_set_source_row_fields_invalid")
        if raw.get("schema_version") != RISK_SET_SOURCE_ROW_SCHEMA:
            raise ValueError("vsb_risk_set_source_row_schema_invalid")
        security_id = validate_security_ids([str(raw.get("security_id") or "")])[0]
        company_id = _nonempty_text(raw.get("company_id"), field="source_company_id")
        ...
        if str(raw.get("listing_country") or "").upper() != "US":
            raise ValueError("vsb_risk_set_source_non_us_listing")
        if str(raw.get("security_class") or "").upper() != "COMMON_EQUITY":
            raise ValueError("vsb_risk_set_source_security_class_invalid")
        for field in ("primary_listing", "active_tradable", "unique_security_mapping"):
            if raw.get(field) is not True:
                raise ValueError("vsb_risk_set_source_required_true:" + field)
        ...
        if membership.date() != decision_date or observed.date() != decision_date:
            raise ValueError("vsb_risk_set_source_not_same_day")
        if membership > observed or observed > available or available > cutoff:
            raise ValueError("vsb_risk_set_source_time_order_invalid")
        ...
        if source_receipt_sha not in receipt_hashes or identity_receipt_sha not in receipt_hashes:
            raise ValueError("vsb_risk_set_source_row_receipt_unbound")
    ...
    if len(set(security_ids)) != len(security_ids):
        raise ValueError("vsb_risk_set_source_duplicate_security_id")
    if len(set(trading_items)) != len(trading_items):
        raise ValueError("vsb_risk_set_source_duplicate_trading_item_id")
    if len(set(primary_listing_ids)) != len(primary_listing_ids):
        raise ValueError("vsb_risk_set_source_duplicate_primary_listing_id")
    return sorted(output, key=lambda row: str(row["security_id"]))
...
def _validate_market_source_rows(
    ...
    *,
    ...
    if not rows:
        raise ValueError("vsb_market_source_rows_required")
    ...
    for raw in rows:
        if not isinstance(raw, Mapping) or set(raw) != MARKET_SOURCE_ROW_FIELDS:
            raise ValueError("vsb_market_source_row_fields_invalid")
        if raw.get("schema_version") != MARKET_SOURCE_ROW_SCHEMA:
            raise ValueError("vsb_market_source_row_schema_invalid")
        security_id = validate_security_ids([str(raw.get("security_id") or "")])[0]
        if security_id not in listing_by_security:
            raise ValueError("vsb_market_source_security_outside_risk_set")
        ...
        if trading_item_id != listing_by_security[security_id]:
            raise ValueError("vsb_market_source_trading_item_mismatch")
        session_date = _date_value(raw.get("session_date"), field="market_session_date")
        if session_date > decision_date:
            raise ValueError("vsb_market_source_after_decision_date")
        key = (security_id, session_date)
        if key in seen:
            raise ValueError("vsb_market_source_duplicate_security_session")
        seen.add(key)
        ...
        if observed > available or available > cutoff:
            raise ValueError("vsb_market_source_time_order_invalid")
        ...
        if close <= 0:
            raise ValueError("vsb_market_source_close_must_be_positive")
        if volume < 0:
            raise ValueError("vsb_market_source_volume_must_be_nonnegative")
        source_receipt_sha = str(raw.get("source_receipt_sha256") or "")
        if source_receipt_sha not in receipt_hashes:
            raise ValueError("vsb_market_source_row_receipt_unbound")
    ...
    for security_id in sorted(listing_by_security):
        if counts.get(security_id, 0) < MIN_HISTORY_SESSIONS:
            raise ValueError("vsb_market_source_insufficient_60_session_history:" + security_id)
        if latest_dates.get(security_id) != decision_date:
            raise ValueError("vsb_market_source_decision_session_missing:" + security_id)
    return sorted(output, key=lambda row: (str(row["security_id"]), str(row["session_date"])))
...
def _validate_receipts(
    ...
    *,
    ...
    if not receipts:
        raise ValueError(f"vsb_{label}_source_receipts_required")
    output: list[dict[str, Any]] = []
    hashes: set[str] = set()
    for raw in receipts:
        if not isinstance(raw, Mapping):
            raise ValueError(f"vsb_{label}_source_receipt_mapping_required")
        binding = dict(raw)
        validate_source_receipt_binding(binding)
        provider = str(binding.get("provider") or "").strip().upper()
        if fixture:
            if provider != FIXTURE_PROVIDER_NAME:
                raise ValueError(f"vsb_{label}_fixture_provider_invalid")
        elif provider not in REAL_PROVIDER_NAMES:
            raise ValueError(f"vsb_{label}_provider_not_ciq")
        retrieved_at = _timestamp_value(binding.get("retrieved_at"), field=f"{label}_retrieved_at")
        if retrieved_at > cutoff:
            raise ValueError(f"vsb_{label}_source_retrieved_after_as_of")
        if retrieved_at < completed_close:
            raise ValueError(f"vsb_{label}_source_retrieved_before_completed_close")
        digest = str(binding["raw_receipt_sha256"])
        if digest in hashes:
            raise ValueError(f"vsb_{label}_source_receipt_duplicate")
        hashes.add(digest)
...
def _require_completed_primary_close(*, cutoff: datetime, decision_date: date) -> datetime:
    ...
    if decision_date.year != 2026:
        raise ValueError("vsb_source_execution_calendar_out_of_scope")
    if decision_date != local_cutoff.date():
        raise ValueError("vsb_source_decision_date_must_match_nyse_local_as_of_date")
    if decision_date.weekday() >= 5 or decision_date.isoformat() in NYSE_2026_CLOSED_DATES:
        raise ValueError("vsb_source_decision_date_not_nyse_session")
    completed_close_local = datetime.combine(decision_date, CONSERVATIVE_PRIMARY_CLOSE, tzinfo=NYSE_TZ)
    if local_cutoff < completed_close_local:
        raise ValueError(
            "vsb_source_primary_close_not_completed:"
            f"required={completed_close_local.isoformat()};observed={local_cutoff.isoformat()}"
        )
    return completed_close_local.astimezone(timezone.utc)


def _nonempty_text(value: Any, *, field: str) -> str:
    text = str(value or "").strip()
    if not text:
        raise ValueError(f"vsb_{field}_required")
    return text


def _date_value(value: date | str | Any, *, field: str) -> date:
    if isinstance(value, datetime):
        raise ValueError(f"vsb_{field}_date_required")
...
def _timestamp_value(value: Any, *, field: str) -> datetime:
    ...
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise ValueError(f"vsb_{field}_timezone_required")
...
def _finite(value: Any, *, field: str) -> float:
    try:
        ...
        raise ValueError(f"vsb_{field}_finite_required") from exc
    if not isfinite(parsed):
        raise ValueError(f"vsb_{field}_finite_required")
    return parsed
