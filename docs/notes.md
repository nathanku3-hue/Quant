## 2026-08-12 AI-Native Runtime Closure / Constitution–Runtime Split

- Authority cut only: `codex/pit-source-authority-1` worktree; `public main = NON_AUTHORITY_UNTIL_MERGE`. Stale audits that treat main or older cuts as controlling are invalid for current research truth.
- Correct status: `AI_NATIVE_RESEARCH_CONSTITUTION=LOCKED`; `AI_NATIVE_DECISION_SEMANTICS=LOCKED`; `FAMILY_LEVEL_FIRST_FAIL=IMPLEMENTED_IN_FTK`; `UNIVERSAL_PREFLIGHT_ENFORCEMENT=NOT_IMPLEMENTED`; `CROSS_FAMILY_RDV_ALLOCATOR=NOT_IMPLEMENTED`; `OUTCOME_CAPABILITY_FIREWALL=PARTIAL`; runtime=`PARTIAL`.
- Do **not** claim AI-native loop absent; do **not** rename scientific L5→SCIENCE_S5; L0–L11/P1–P4 remain the scientific namespace.
- D1–D9 split: constitution LOCKED; FTK implementation IMPLEMENTED (final econ first-fail D6; D8/D9 NOT_REACHED); universal runner NOT_IMPLEMENTED.
- OpportunityKernelV1 + result-first preflight + ExecutionFeasibilityV1 contracts are LOCKED; zero universal Python consumers means legacy outcome paths (e.g. `research/backtest_runner.run_research_backtest`) can still bypass ScientificDecisionReceipt / L5 auth / trial debit / RDV.
- Risk provenance gap: interceptor practical ALLOW|BLOCK with order/portfolio/default volatility fallbacks; locked feasibility requires ALLOW|BLOCK|UNRESOLVED and multi-margin vector authority.
- Top-3 GQ now: constitutional enforcement closure; action-boundary clock + risk-state provenance; endogenous observability. Then: authority reproducibility, common-mode raw custody, diagnostic laundering, L5 compare-set integrity, correlated abstention, AI gate gaming, temporal typing of current vs historical SoT.
- Narrow seams only: universal outcome gate; action-boundary packet; fail-closed risk provenance; fresh-checkout authority-closure test; L5 comparison-set receipt. No strategy/platform reopen.
- Canonical docs: `docs/architecture/ai_native_runtime_closure_audit_20260812.md`, `docs/architecture/ai_native_runtime_status_v1.json`.
...
- Basis status precedence: `Q missing + M not observed => Q_AND_M_MISSING`; `Q missing + M observed => Q_UNOBSERVED`; `Q observed + M warmup => M_WARMUP`; `Q observed + M missing => M_MISSING_HISTORY`; `Q observed + M observed => ELIGIBLE_COMPLETE`.
- Arm law: Q is selectable whenever Q is observed; M-perp and Q+M-perp are selectable only on Q∩M. Every non-observable arm action is `ABSTENTION`, `selected=false`, `risky_weight=0`, and the W3 row remains in the opportunity denominator.
- Rank law: `rank_Q_arm` uses all Q-observed W3 names on date `t`. For residualization, recompute `rank_Q_joint` and `rank_M_joint` on exactly Q∩M; then fit `rank_M_joint = a_t + b_t*rank_Q_joint + epsilon_t` and define `M_perp=epsilon_t`. No temporal fit, outcome input, or cross-date coefficient reuse.
- Q source-observability law in AO-K0A is source-state only: latest planned S0 transition at/before decision date must be admitted by the existing S0 admission/quarantine law; a quarantined planned transition sets Q source state unobserved until a later admitted transition. AO-K0A does not borrow old Rule100 factor artifacts or silently invent a replacement numeric Q.
- M observability law: exact W3 `(security_id,trading_item_id)` requires 60 continuous W3-eligible sessions with finite `SP_TOTAL_RETURN` in exact W3 market custody. Global history shorter than 60 sessions=`M_WARMUP`; otherwise an incomplete exact-listing window=`M_MISSING_HISTORY`. Alternate listing, ticker/entity/PERMNO, or missing-return bridge is forbidden.
- Full-W3 evaluation law after separately authorized outcome opening: `Recall = selected∩winner / winner_full_W3`; `Breadth = selected / W3`; abstain winners are foregone upside; abstain bottom-tail names are avoided catastrophe. Complete-case denominator and observed-subset renormalization are forbidden.
- Capital law: no security-level or peer return imputation. ABSTENTION leaves risky weight at zero; residual capital uses canonical economic cash (`build_economic_cash_frames`); opportunity comparison uses PIT equal-weight full W3 (`build_pit_equal_weight_benchmark`).
- Source-only preflight: W3 346-session mean eligible count=`4822.994219653179`; pre-W6 weekly matrix=`310329` rows, SHA-256=`bd36a6305f38ff68c57f6ccfb9d3481be6fd42d2288128ef9ec3eb3cc12df5cf`; statuses=`223367 ELIGIBLE_COMPLETE / 17767 M_MISSING_HISTORY / 30270 M_WARMUP / 36068 Q_AND_M_MISSING / 2857 Q_UNOBSERVED`; missingness row deletion=`0`; W6/outcome/provider reads=`0`.
- Historical ~79.49% common coverage is not recertified and is not an active gate. Under the allowed source set, old Rule100 numeric-Q observability is not uniquely reconstructable from admitted S0; never tune a source definition merely to recover a prior coverage percentage.
- AO-K0A trial charge=`0`; `financial_alpha_evidence=0`; W6 untouched. AO-K0B remains blocked until numeric Q and scalar Q+M-perp composition are explicitly source-bound, followed by exact development input + uninspected-label hashing.

## 2026-08-10 PREBREAKOUT W3 PIT Authority Formula / Custody Registry

- Family/risk-set identity: `PREBREAKOUT_DISCOVERY_v1 / PREBREAKOUT_US_PRIMARY_COMMON_DATE_LOCAL_V1`; implementation=`research/prebreakout_pit_v1/authority.py`; architecture=`docs/architecture/prebreakout_pit_authority_v1.md`.
- Exact risky identity law: `security_id := CIQSEC:IQ<digits>` and `listing_id := trading_item_id` with `spt_instrument_item_id == "SPT" + trading_item_id`. Ticker is display-only; company entity ID and PERMNO are not risky identity; alternate-listing backfill is forbidden.
- Source-authority gate: `date_local_membership_query AND source_population_complete AND corporate_action_coverage_complete AND NOT(current_survivor_back_projection_used OR current_primary_back_projection_used OR alternate_listing_backfill_used OR ticker_fallback_used OR company_entity_fallback_used OR permno_fallback_used)`.
- Historical capture gate: `capture_mode=HISTORICAL_PIT_DATE_LOCAL => historical_as_of_mechanically_bound=true`; prospective same-day capture requires that historical flag false. Row timing law for both candidate and corporate-action state is `observed_at <= available_at <= as_of` and candidate `membership_as_of_date == decision_session_date`. Every source receipt also requires `observed_range_start <= observed_range_end <= decision_session_date`; prospective receipts additionally require `retrieved_at <= as_of`, while historical wall-clock retrieval may be later only under the mechanically bound as-of contract.
- Date-local primary law: `PRIMARY_DATE_LOCAL` requires proof `DATE_LOCAL_PROVIDER_PRIMARY` or `UNIQUE_DATE_LOCAL_QUALIFYING_LISTING`; `AMBIGUOUS_DATE_LOCAL` maps to `AMBIGUOUS_PRIMARY_LISTING`; no current Primary Issue repair is legal. A packet with multiple eligible listings for one company fails closed.
- Corporate-action law: exactly one action row per exact `(CIQSEC, trading_item)` candidate. `CLEAR` and `PENDING_TERMINAL` (effective strictly after decision session) may remain eligible; `EFFECTIVE_TERMINAL` maps to `CORPORATE_ACTION_TERMINAL_EFFECTIVE`; `UNRESOLVED` maps to `CORPORATE_ACTION_UNRESOLVED`; incomplete action coverage blocks the packet.
- Deterministic eligibility order: non-US → non-common → ambiguous-primary → non-primary → inactive → unresolved corporate action → effective terminal action → eligible. There is no post-exclusion identity/listing rescue.
- B-1 proof law: W3 consumes exact frozen W2 `breakout_contract_sha256=94c8756e11d4e31cbf4d6ca4953b63c83306f4b357a42f0b70c94ae3fd261e71`; any other non-null hash fails closed. W2 supplies exact `B` and exact observed-listing-session `B-1`; W3 requires `authority.decision_session_date == B-1` and does not derive the prior session. Exact eligible identity yields `PIT_ELIGIBLE_B_MINUS_1`; exact excluded identity yields its deterministic exclusion; exact identity absent from a source-complete packet yields `NOT_IN_DATE_LOCAL_SOURCE_POPULATION`.
- Smoke firewall: `display_symbol_used_for_logic=false`; every proof seals `statistical_weight=0`, `promotion_denominator_weight=0`, `outcome_access_performed=false`, `financial_alpha_evidence=0`, `capital_authority=NONE`.
- Historical/current split: `docs/context/e2e_evidence/prebreakout_pit_w3_mu_sndk_20260810.json` is `HISTORICAL_PRE_W2_CUSTODY_ONLY`; its `BREAKOUT_CONTRACT_UNBOUND` reason is retained only for historical reproducibility and does not satisfy current W2.
- Current MU/SNDK state: no W3-authorized retained CIQ bytes resolve exact CIQSEC+Trading Item plus date-local exact-listing close/corporate-action authority, so real B/B-1 is not computed. Both are `B_MINUS_1_PIT_AUTHORITY_UNAVAILABLE`, zero statistical/promotion weight, and unavailable is never a pass. Current evidence=`docs/context/e2e_evidence/prebreakout_pit_w3_current_gate_20260810.json`.
- Future acquisition manifest=`docs/architecture/prebreakout_pit_acquisition_manifest_v1.json`, SHA-256 `f3cac6961707fc653c00016ff715fb7250236ad443854051a387ab330473dc3f`: full date-local U.S. primary-common population + exact identity/listing + availability + complete corporate actions + historical-as-of binding + exact-listing market history. W9 may share only byte-identical raw identity/market acquisition on identical requests; family/risk-set admission remains separate. Provider capture today is forbidden.
- Validation: W3 focused=`17/17 PASS`; W2+W3 frozen-handshake=`24/24 PASS`; adjacent W2/W3 + W9 CIQ admission + historical identity/lifecycle=`40/40 PASS`; compile, JSON/hash/state and Git whitespace checks PASS.

## 2026-08-10 VSB M0 Core Formula / Custody Registry

- Active research-family WIP=`3`: `CYCLE_RESONANCE_v1` + `VOL_SQUEEZE_BREAKOUT_v1` + ETF-first `SECTOR_ROTATION_ALPHA_v1`. Evidence qualification may parallelize; current portfolio/capital-policy authority remains singular. VSB has priority on any shared implementation/custody capacity.
- Family contract: `VOL_SQUEEZE_BREAKOUT_v1 / VSB_US_PRIMARY_COMMON_DAILY_V1 / VSB_RIGHT_TAIL_10D_TOP5_V1`; allowed observations exactly `{market.close, market.total_return_1d, market.volume}`; expectation and claim surfaces empty. Family contract is hash-bound into Alpha PIT artifacts/sessions/outcome labels.
- PIT packet law: every market row binds exact `CIQSEC:<security>` + trading-item identity + source receipt; no ticker/PERMNO/entity fallback, no future row, no extra risk-set name, exact decision-date row required, and at least 60 observed sessions through decision date `t`. Real provider acquisition remains outside the packet module.
- `rv20_i,t := sample_std(ret1d_i,t-19..t, ddof=1)`; `rv60_i,t := sample_std(ret1d_i,t-59..t, ddof=1)`; `compression_i,t := ln(rv60_i,t / rv20_i,t)` with both vols finite and `>0`.
- `prior_high20_i,t := max(close_i,t-20..t-1)`; `breakout_i,t := ln(close_i,t / prior_high20_i,t)` with positive finite domain.
- `prior_volume_median20_i,t := median(volume_i,t-20..t-1)`; `volume_expansion_i,t := ln(volume_i,t / prior_volume_median20_i,t)` with positive finite domain. Zero/nonfinite required domains produce `INSUFFICIENT_OR_INVALID_M0_HISTORY`; they are not imputed or repaired.
- Trigger: `M0_TRIGGER_i,t := compression_i,t > 0 AND breakout_i,t > 0 AND volume_expansion_i,t > 0`.
- Cross-sectional rank semantics are frozen to average one-based tie rank divided by finite cross-section count: `r_x(i,t) := average_rank(x_i,t among finite READY names) / N_ready`. `raw_score := (r_compression + r_breakout + r_volume_expansion)/3`; `forecast_score := raw_score` only when `M0_TRIGGER`, else `0`.
- Search identity: `implementation_id=VSB_M0_EQUAL_RANK_20_60_20_v1`; `search_family_id=VSB_M0_SEARCH_v1`; `trial_budget_max=1`; `material_trials_consumed=1`. Any material feature/window/threshold/weight/risk-set/label/acceptance change requires a new charged implementation before evaluation.
- W7 role freeze: `confirmation_role_id=VSB_CONFIRMATION_v1`; `retune_authority=NONE`; `prebreakout_authority=NONE`. The guardian contract hash binds the frozen family/implementation/search/10d-label identity plus `min_matured_dates=20`, `lift_threshold=1.0`, and the exact bootstrap law; every newly sealed VSB prediction carries that guardian hash.
- Confirmation formula: for matured dates `T`, `winner_recall_10d := Σ_t winner_support_hits_t / Σ_t winner_count_t`; `support_breadth := mean_t(support_count_t / risk_set_count_t)`; `winner_recall_lift_10d := winner_recall_10d / support_breadth` with zero breadth mapped to lift `0`. Gate requires `|T|>=20`, observed lift `>1`, and the 80% moving-block-bootstrap lower bound `>1`.
- Bootstrap law: unit=`decision date`; chronological non-circular blocks of length `10`; eligible starts `0..N-10`; sample blocks with replacement until at least `N` observations and truncate to `N`; `10000` replicates; seed=`20260810`; deterministic Type-7 10th/90th percentiles form the 80% two-sided interval. Before 20 matured primary dates the guardian returns only the eligibility count and exposes no lift/CI values.
- W7 receipt seam: W7 consumes only separately authorized **VSB-specific matured 10d evaluation receipts** from the untouched/prospective VSB evaluation path. Each receipt binds the sealed VSB prediction-batch hash, VSB evaluation-receipt hash, frozen VSB identities/guardian hash, `prediction_before_label_open=true`, zero custody violations, and exact `ceil(0.05*risk_set_count)` winner count. PREBREAKOUT W6 is not a VSB evaluator and its outputs are invalid W7 inputs. W7 never fetches outcomes or constructs labels.
- Prediction timing: `prediction_made_at > knowledge_cutoff(as_of)` strictly. Prediction objects bind input/PIT-packet/feature/model hashes plus the frozen guardian contract and retain `outcome_status=UNMATURED_NOT_EVALUATED`, `financial_alpha_evidence=0`, `capital_authority=NONE`, `broker_orders=FORBIDDEN`, `parent_child_mutation=FORBIDDEN`, `retune_authority=NONE`, `prebreakout_authority=NONE`.
- Source-admission law: `research/vol_squeeze_breakout_v1/source.py` performs no provider/network acquisition. It accepts only already-landed same-day CIQ receipts, freezes U.S./common/primary/active/unique identity evidence, rejects growth/Rule100/current-survivor/alternate-listing/legacy-identity/liquidity shortcuts, then emits the exact risk-set + market-history artifacts consumed by the family packet.
- Prediction-tape law: `research/vol_squeeze_breakout_v1/ledger.py` is append-only and hash-chained with an exclusive-create writer lock, one prediction per decision date, fsync, reload verification, monotonic custody time, and fail-closed tamper/partial-line handling. Fixture tape tests do not count as a real evidence clock.
- Implementation is now parked: `research/vol_squeeze_breakout_v1/contracts.py`, `source.py`, `pit_packet.py`, `features.py`, `model.py`, `runner.py`, `ledger.py`, `guardian.py`; tests under `tests/vol_squeeze_breakout_v1/`. W7 focused matrix=`22/22 PASS`; final live-byte VSB + Alpha PIT + CRV1 matrix=`49/49 PASS`; AOV directory=`167/167 PASS`; selected W7 modules compile PASS. `worker_assignment=NONE`; no provider/outcome/capture/prediction-clock action is active.
...
## AOV Winner Capture Diagnostic v0 + VOL_SQUEEZE_BREAKOUT_v1 Formula Registry (2026-08-10)

- Winner-diagnostic anchor law: `A = {first Rule100 target row} ∪ {t : max_i |w_rule100(i,t)-w_rule100(i,t-1)| > 1e-12}`. Repeated daily forward-filled copies of one target state are not separate decision episodes.
- Forward winner label: `F_H(i,t) = Π_{s=t+1}^{t+H}(1+r(i,s))-1` over the next `H` observed replay sessions, `H∈{10,20}`. Mature only when the full horizon is already retained. Winner count `K_t=ceil(0.05*N_t)`; sort `F_H DESC, security_id ASC` for ties.
- Winner access metrics: `winner_recall_H = Σ_t |W_H(t)∩support_rule100(t)| / Σ_t |W_H(t)|`; `selection_breadth = mean_t(|support_rule100(t)|/N_t)`; `winner_recall_lift = winner_recall/selection_breadth` when breadth is positive.
- Funnel law: risk-set access → Rule100 positive target (`sizing_eligible` support) → Parent positive target → Parent positive executed weight on the first future session (`entry_lead`) → first-session capital share → on-time positive Parent contribution. A winner entered later in the horizon is `late_entry_captured`, never retroactively credited as entry lead.
- Child clipping on winner episode: `gross_giveup = Σ_H[(w_parent_exec-w_child_exec)*r_i]`. False-positive downside avoidance: for Parent-supported nonwinners with `F_H<0`, `avoided_loss=max(Σ_H(w_child_exec-w_parent_exec)*r_i,0)`. Overlapping horizon sums are diagnostic only.
- Whole-stage non-overlap reconciliation: `gap_i = Σ_t[(w_parent_exec(i,t)-w_child_exec(i,t))*r(i,t)]`; `Σ_i gap_i` must equal retained `Parent gross_return_sum - Child gross_return_sum` within `1e-12`.
- Regime law matches CIQ features exactly: each security computes `sma200` on its own **observed** close rows; query-grid NA placeholders do not count as sessions. `trend_slow=+1` if `close>=sma200`, else `-1`; `regime=date-local mean(trend_slow)`; buckets are sign-only positive/neutral/negative, with unavailable kept unavailable.
- Retained A2 reproduction: 10d recall `26.6667%` at breadth `20.3901%`; 20d recall `20.0000%` at breadth `19.1489%`. Parent−Child gross gap `1.373092pp`; Child cost savings `0.048211pp`; full-stage top-five winners `OPHC/SMTI/CSTL/NTRA/TLSI` explain `92.5781%` of the gross gap. Historical diagnostic only; `financial_alpha_evidence=0`.
- VSB risk-set law: `VSB_US_PRIMARY_COMMON_DAILY_V1` is a date-local U.S.-listed primary-common security set with exact `CIQSEC` + trading-item identity, active/tradable eligibility at cut, `>=60` prior observed market sessions, and no growth/fundamental/current-survivor filter. No V1 liquidity threshold.
- VSB M0: `rv20=std(ret1d[t-19:t])`, `rv60=std(ret1d[t-59:t])`, `compression=ln(rv60/rv20)`; `breakout=ln(close_t/max(close[t-20:t-1]))`; `volume_expansion=ln(volume_t/median(volume[t-20:t-1]))`. Trigger iff all three are `>0`. Score is the equal-third mean of date-local percentile ranks when triggered, else `0`; no fitted coefficients or top-N tuning.
- VSB labels use the same deterministic top-5% law on next 10 observed sessions primary and next 20 secondary. First M0 acceptance requires a **new untouched/prospective freeze**, at least 20 matured 10d decision dates, `winner_recall_lift_10d>1`, and the preregistered 80% moving-block-bootstrap lower bound `>1` (`block=10` dates, `replicates=10000`, `seed=20260810`). A1/A2 and SNDK/MU have zero acceptance weight.
- Family isolation law: exact immutable six-field `FamilyDataContract` binds family ID, risk-set spec, primary label spec, and allowed observation/expectation/claim surfaces into Alpha PIT sessions/artifacts. VSB permits only `market.close`, `market.total_return_1d`, `market.volume`; claims/expectations are empty. No registry/plugin/provider platform is introduced.
- VSB source-admission law: one decision date must equal the **New-York-local** date of prospective `as_of`; the date must be an admitted 2026 NYSE session and the source cut is blocked until a conservative `16:00 America/New_York` completed-close boundary. Risk-set and market source receipts must be retrieved at/after that close and at/before `as_of`. Every source membership row proves `US + COMMON_EQUITY + primary_listing + active_tradable + unique_security_mapping` before the cutoff and binds source/identity receipt hashes. `growth_filter_applied`, `rule100_filter_applied`, `current_survivor_back_projection_used`, `alternate_listing_backfill_used`, `legacy_identity_fallback_used`, and `liquidity_threshold_applied` must all be `False`. Market rows must bind the exact admitted security/trading-item pair, remain at/before the decision date, carry finite close/return/volume, and provide `>=60` observed sessions with the decision session present.
- VSB prediction-tape chain: `chain_hash_n = domain_hash(VOL_SQUEEZE_BREAKOUT_V1:PREDICTION_LEDGER_LINK, canonical(link_body_n))`, where `previous_chain_hash_0 = 0^64` and `previous_chain_hash_n = chain_hash_{n-1}` thereafter. Exactly one prediction batch per decision date is allowed. Each append verifies the sealed batch first, acquires an exclusive-create writer lock, appends one complete JSONL line with `O_APPEND`, fsyncs, then reloads/verifies the full chain. An already-existing writer lock fails closed and is never deleted by a losing writer.
- VSB tape authority: every link remains `evaluation_status=UNMATURED_NOT_EVALUATED`; persistence does not create outcome visibility, financial Alpha evidence, capital authority, broker authority, or permission to alter the frozen one-trial M0. The first empirical read remains the preregistered new prospective `>=20` matured-10d winner-recall-lift gate.

## PREBREAKOUT W5 Trial-1 M0 Formula Registry (2026-08-10)

- Trial identity: `PREBREAKOUT_TRIAL_1_M0` / `PREBREAKOUT_TRIAL1_M0_MARKET_EARLY_WARNING_V1`; current state is **uncharged**, `material_trials_consumed=0/8`, development not run, W6 closed.
- Market-only feature law in `research/prebreakout_discovery_v1/trial1_m0.py`: `prior_high20_t=max(close[t-20:t-1])`; near-high gate=`0.95*prior_high20_t <= close_t <= prior_high20_t`; near component=`clip((close_t/prior_high20_t-0.95)/0.05,0,1)`. The upper bound forbids post-breakout rescue because W2 breakout B is strict `close_t > prior_high20_t`.
- Volatility component: `rv20=sample_std(ret1d[t-19:t],ddof=1)`, `rv60=sample_std(ret1d[t-59:t],ddof=1)`; gate=`rv20<rv60`; component=`clip(log(rv60/rv20)/log(2),0,1)`.
- Volume-pressure component: `recent5=median(volume[t-4:t])`, `prior15=median(volume[t-19:t-5])`; gate=`recent5>prior15`; component=`clip(log(recent5/prior15)/log(2),0,1)`.
- Trigger/score: `trigger=near_high_gate AND vol_compression_gate AND volume_pressure_gate`; `forecast_score=mean(three capped components)` when triggered, else `0`. Fitted parameters=`0`; calibration=`NONE`; insufficient/invalid history abstains at zero with no imputation.
- Split law: expanding training only for Trial-1, minimum `126` sessions, embargo=`20`, four OOS folds of `20` sessions each; exact spine requires at least `226` sessions. A later rolling-mode read is a material training-window variant and requires another charged trial rather than silent robustness tuning.
- Cross-sectional holdout: deterministic canonical-CIQSEC hash `mod 5`, remainder `0`, fixed seed; approximately 20%. Holdout labels never tune. MU/SNDK can be scored only through generic zero-statistical-weight traces.
- Control law: full date-local positive-weight non-holdout W3-eligible denominator; breadth=`flagged_count/eligible_count`. W4 compatibility uses constant `trial1_control_stratum=ALL_W3_ELIGIBLE`, so all same-session positive-weight ordinary controls are retained with no sector/size map, sampling, or rematch degree of freedom.
- Primary development objective: 20d top-5% temporal-OOS winner-recall lift versus date-local flag breadth, then median effective TTFLD with misses=`0`; comparison is lexicographic. CAGR/Sharpe are not primary. Fewer than two informative folds or no eligible winner episodes => `NULL`, and a charged Trial-1 still costs one trial.
- Pre-charge custody: exact source manifest must bind W2/W3 risk-set/label law, market payload, W3 PIT bundle, frozen W4 control definition, W4 development-label/episode custody, decision spine, and source receipts while `development_label_visibility=HASHED_NOT_INSPECTED`, holdout tuning forbidden and W6 absent. Only then may W2 `TRIAL_OPEN` consume `1/8` before result inspection.
- Frozen identities: uncharged declaration SHA-256=`02b0e07445bb91ab7fd4c192abbd26b112facf75cf4923d4ddd8778ea5a062c1`; split SHA-256=`7a0b77fbb95fc39a86723fd3f79d69d6fd88c12f73db65fd3224ff2723fcc9af`; control-definition SHA-256=`144f3c08ba5ff576cd1d4a702129893f9b08f9b58b4d4a6e21ad385a7d7c44ba`; code-bundle SHA-256=`92f51a098f2420fd5d76b7307d0aad75a76d0cf10a81934aa48a15a20f43b2bd`.

## ECONPHYSICS S0 Structured-State Formula / Evaluation Registry (2026-08-10)

- Request identity: `ECONPHYSICS_PREBREAKOUT_S0_STRUCTURED_STATE_TRANSITION_PROOF_v1`; request manifest=`docs/architecture/econphysics_prebreakout_s0_structured_request_v1.json`. `CAPTURE GO / S0 ONLY` was granted and exercised. Stage 1 completed `53,200` exact `IQ_PERIOD_END/FQ0` requests (`266 × 200`) before the execution host's `300s` hard timeout; no atomic raw CSV/receipt landed, so admitted S0 rows remain `0` and mechanism evidence remains absent.
- Capture law: exact W3 date-local eligible pair probes only; `IQ_PERIOD_END/FQ0` is the cheap state-change detector. Five-quarter payload is requested only on adjacent nonmissing advancing FQ0 period changes and is restricted to `IQ_PERIOD_END`, `IQ_TOTAL_REV`, `IQ_INVENTORY`, `IQ_OPER_INC`, `IQ_CAPEX_BNK` with `FilingVer=Original` and USD-thousands units.
- Direction primitive: `dir(x_t,x_c)=+1` if `x_t>x_c`, `-1` if `x_t<x_c`, else `0`; missing either side => unobserved. Each evidence item carries `latest_vs_prior=dir(FQ0,FQ-1)` and `yoy_direction=dir(FQ0,FQ-4)` with no fitted epsilon or threshold.
- Inventory economic sign is inverted for level and inventory/revenue evidence because falling inventory burden is the positive economic direction. Demand/revenue, operating margin and capex-cycle evidence keep the raw upward-positive sign.
- Ratio admission: `inventory_to_revenue_q = IQ_INVENTORY_q / IQ_TOTAL_REV_q`; `operating_margin_q = IQ_OPER_INC_q / IQ_TOTAL_REV_q`; numerator/denominator must come from the same validated relative-period row and shared `USD_THOUSANDS` unit, and revenue must be positive. Otherwise the ratio is `UNOBSERVED`; no imputation.
- State aggregation: contradictory material evidence becomes `MIXED` and emits no forecast direction. No lawful evidence becomes `UNOBSERVED`. `SUPPLY_CAPACITY_STATE` remains `UNOBSERVED` in S0; `IQ_CAPEX_BNK` is emitted only as `CAPEX_CYCLE_EVIDENCE_V1`.
- Self-falsifier targets use only the next admitted PIT snapshot: inventory target=`-dir(next inventory/revenue, current inventory/revenue)`; demand target=`dir(next revenue,current revenue)`; margin target=`dir(next operating_margin,current operating_margin)`.
- Evaluation baseline: among evaluable target rows, `no_information_baseline_hit_rate = max(class_count[-1],class_count[0],class_count[+1]) / N`; `directional_hit_rate=exact_direction_matches/N`; `lift=directional_hit_rate/no_information_baseline_hit_rate`. Contradiction means prediction×actual `<0`; nonzero directional association is the mean prediction×actual over nonzero/nonmissing pairs.
- Temporal gate: four deterministic contiguous feature-date folds; target fold supports mechanism iff `N>0`, `lift>1`, and directional association `>0`. S0 node target passes only with support in at least `3/4` folds; fewer than three informative folds=`UNOBSERVED`, otherwise failure=`FAILED`. Deterministic XS holdout is `sha256("ECONPHYSICS_S0_XS_HOLDOUT_V1|" + CIQSEC) mod 5 == 0` and is corroboration only, never tuning.
- Output authority: all reports explicitly assert no fit/tuning, market-data access, equity-outcome access, W6 access or selection. `financial_alpha_evidence=0`; S0 mechanism evidence does not exist until a complete provider capture lands and is evaluated.
- Transport stop law after the first real attempt: the all-or-nothing Stage 1 historical attempt is `INCOMPLETE_HOST_TIMEOUT_NO_RAW_ADMISSION`. It was later superseded as current execution truth by restartable atomic provider-batch custody; the historical stop/no-rescue evidence remains immutable.

## ECONPHYSICS S0 Economic Dynamics Diagnostic Registry (2026-08-11)

- Diagnostic implementation=`research/econphysics_prebreakout_v1/dynamics_diagnostic.py`; runner=`scripts/econphysics_prebreakout_s0_dynamics_diagnostic.py`; real output=`data/prebreakout/analysis/econphysics_s0_economic_dynamics_diagnostic_v2.json`.
- Reuse law: exact admitted Original-filing S0 snapshots, predecessor-period gate, four deterministic temporal folds, deterministic CIQSEC XS holdout, and existing next-PIT economic targets. No market/winner/W6/K/threshold/provider surface is read.
- Economic level: revenue=`IQ_TOTAL_REV`; margin=`IQ_OPER_INC/IQ_TOTAL_REV`; inventory normalization level=`-(IQ_INVENTORY/IQ_TOTAL_REV)` so positive change means economic improvement.
- Primitive operators: `LEVEL_STATE_PERSISTENCE=sign(e0-e4)`; `DELTA_PERSISTENCE=sign(e0-e1)`; `DELTA_MEAN_REVERSION=-sign(e0-e1)`; `DELTA2_ACCELERATION=sign((e0-e1)-(e1-e2))`, with `e0/e1/e2/e4 = FQ0/FQ-1/FQ-2/FQ-4` economic levels.
- Representation-conditioned operators: `M0_STATE_MEAN_REVERSION=-frozen_M0_prediction_direction`; `M1_STATE_MEAN_REVERSION=-frozen_M1_prediction_direction`. These reverse only the dynamics operator; they do not mutate M0/M1 representation bytes.
- Stable node support remains the pre-existing absolute gate: `lift_vs_no_information_baseline>1 AND directional_association>0` in at least `3/4` temporal folds; XS is corroboration only. No best-operator selection is performed.
- Real routing: inventory survives via `DELTA_MEAN_REVERSION`, `M0_STATE_MEAN_REVERSION`, `M1_STATE_MEAN_REVERSION`; operating margin survives via `M1_STATE_MEAN_REVERSION`; revenue has no fixed-family survivor. Therefore integrated route=`NODE_SPECIFIC_DYNAMICS_SURVIVORS`, not global `OBSERVABLE_INSUFFICIENCY`.
- Layer law: upstream causal observables=`orders/backlog`, `pricing/mix`, `utilization/capacity`, `channel inventory`; downstream expectation observables=`revisions`, `guidance`, `consensus`. Do not mix the two when diagnosing economic-state dynamics.
- Claim boundary: same-corpus posterior diagnostic only; persistence M0/M1 FAIL remains valid, but the inference “structured fundamentals contain no transition information” is invalid. A future confirmatory operator/representation must be frozen before genuinely untouched/prospective evidence. `financial_alpha_evidence=0`.

## AO-K0 Opportunity Constitution + Minimal Basis Formula Registry (2026-08-11)

- Constitution identity=`ASYMMETRIC_OPPORTUNITY_CONSTITUTION_v1`; first basis experiment=`AO_K0_BASIS_Q_M_QM_V1`; status=`FROZEN_METHODOLOGY_AND_BASIS_ONLY_NO_RESULT_READ`; W6 untouched; `financial_alpha_evidence=0`.
- Universal unit=`OpportunityKernelV1` with required fields: `kernel_id / entry_mode / observable_representation / target / horizon / rationale_class / simple_comparator / upside_channel / downside_failure_mode / hard_falsifier / applicability_abstention / pit_source_contract / search_cost_trial_identity`.
- Broad denominator=`AO_BASIS_US_PRIMARY_COMMON_DATE_LOCAL_V1`: exact date-local U.S. primary-common `CIQSEC + Trading Item`, PIT membership/listing/availability/corporate-action truth, no growth/Rule100/quality/momentum/current-survivor/current-primary/future-member/ticker/entity/PERMNO/alternate-listing filter or repair. Missing arm features remain abstention in the denominator.
- Decision schedule=`last observed NYSE session of each calendar week`; primary target=`F20(i,t)=Π_{s=t+1}^{t+20}(1+r_i,s)-1`; winner set=`top ceil(0.05*|D_t|)`; catastrophic set=`bottom ceil(0.05*|D_t|)`; selection `K_t=ceil(0.05*|D_t|)`; final score tie break=`security_id ASC`; execution=`core.engine.run_simulation` `shift(1)`; cost=`turnover*0.0010`.
- Q primitives: `g1=z_demand`, `g2=z_inventory_quality_proxy`, `g3=z_moat`, `g4=capital_cycle_score`. Each `g_j` is average-tie percentile-ranked date-locally among lawfully observed `D_t` values. `q_present_count>=3`; `q_score=mean(observed g_j percentile ranks)`; otherwise abstain. **No missing primitive neutral fill** in AO-K0 Q.
- M primitive: `m_raw(i,t)=Π_{s=t-59}^{t}(1+r_i,s)-1` over exactly 60 observed sessions on the same admitted security/listing identity. `m_score=pct_rank_t(m_raw)`. No benchmark residualization, sector neutralization, volatility normalization, breakout/volume gate, fitted coefficient or fitted threshold.
- Q+M composition: `qm_score=q_score*m_score` only when both are observable. No learned weights, additive coefficient search, interaction search, threshold search, sector adjustment or nonlinear rescue.
- Arm selection: each arm must supply the frozen `K_t`; if observable count `<K_t`, emit `Q_COVERAGE_FAILURE`, `M_COVERAGE_FAILURE` or `QM_COVERAGE_FAILURE`. Never shrink K or the broad denominator to recover a result.
- Asymmetry definitions: `winner_precision=selected_top5pct/selected`; `winner_recall=selected_top5pct/|winner_set_t|`; `right_tail_enrichment=winner_precision/(|winner_set_t|/|D_t|)`; `false_winner_burden=1-winner_precision`; `catastrophic_left_tail_rate=selected_bottom5pct/selected`; `catastrophic_left_tail_lift=catastrophic_left_tail_rate/(|catastrophic_set_t|/|D_t|)`; `cvar_05=mean(worst ceil(0.05*selected_count) selected-name F20 returns)`.
- Required primary report additionally includes coverage/abstention, forward `p05/p25/median/p75/p95`, winner-recognition lead within prior 20 observed sessions, deterministic equal-weight shadow gross/net, capital-time security-sessions, L1 turnover and cost. Shadow evaluation has no capital authority.
- Search charge: AO-K0 freeze=`0`; first result-bearing joint Q/M/Q+M development read=`1` material basis trial after source bytes + uninspected label hash are frozen, before inspection. Any outcome-visible change to Q/M/Q+M formula, risk set, schedule, K, horizon, label, missingness, execution, cost or primary metrics creates a new version/trial.
- Existing-object routing: econphysics=`specialized fundamental-transition/mechanistic`; inventory mean-reversion=`future diagnostic challenger`; margin=`candidate_not_hard_survivor`; revenue=`PARK`; Rule100/Rule40-style=`quality/compounding control or incumbent representation, not proven Alpha`; standalone momentum requires new continuation identity; VSB stays downstream recognition; dislocation deferred; rerating is payoff path.
- Canonical files: `docs/architecture/asymmetric_opportunity_constitution_v1.md`, `docs/architecture/ao_k0_minimal_basis_freeze_v1.json`, `docs/phase_brief/ao_k0_asymmetric_opportunity_constitution_20260811.md`.

## CRV1 E3 Clock / State-Coordinate Registry (2026-08-12)

- Slice=`CRV1-E3-CLOCK-STATE-COORDINATE-PREFLIGHT-1`; terminal=`CLOCK_CONTRACT_PASS`; programme stage=`E3`; method obligation satisfied=`L2_CLOCK_STATE_COORDINATE_DECLARATION`; outcomes/provider/returns/model/label join=`0`; trial debit=`0`; `financial_alpha_evidence=0`.
- Data-availability law: no canonical CRV1 clock uses `DATA_AVAILABILITY` as its scientific role. Every contributing row/claim must satisfy `available_at <= as_of`; `max_available_at` binds clock evidence. Period end/report/retrieval cadence cannot define state direction or become a free economic lag/horizon.
- Core graph remains `SUPPLY_CAPACITY -> INVENTORY -> PRICING -> UTILIZATION_MARGIN -> EARNINGS_REVISIONS -> EXPECTATION_GAP`. `DEMAND_CLOCK` remains corroborating/state only. `MARKET_CONFIRMATION_CLOCK` is downstream `ACTION_EXECUTION` only and cannot establish or rescue upstream causal state.
- Semantic types: first business clocks remain business economic state; `EARNINGS_REVISIONS_CLOCK=EXPECTATION_STATE`; `EXPECTATION_GAP_CLOCK=MISPRICING_STATE`; both sit inside the coarse v2 `ECONOMIC_STATE` role bucket only because the method has no separate expectation role. Their semantic type is binding and prevents realized-business-state conflation.
- State-coordinate law: supply/capacity=`SUPPLY_CAPACITY_DISCIPLINE_STATE`; inventory=`INVENTORY_BURDEN_NORMALIZATION_STATE`; pricing=`PRICING_POWER_MIX_STATE`; demand=`DEMAND_ORDER_TRAJECTORY_STATE`; utilization/margin=`UTILIZATION_OPERATING_LEVERAGE_MARGIN_STATE`; revisions=`CONSENSUS_REVISION_DIRECTION_STATE`; gap=`ECONOMIC_TRAJECTORY_VS_EXPECTATIONS_GAP_STATE`; market confirmation=`MARKET_RECOGNITION_ACTION_CONFIRMATION_STATE`.
- Anti-aliasing law: capex alone is not capacity; denominator/write-down/scope change alone is not inventory normalization; margin alone is not pricing; revenue alone is not demand when price/mix is unresolved; margin does not imply utilization; current/backfilled consensus is not historical revision state; price/return cannot define expectation gap; market confirmation cannot repair upstream thesis integrity.
- Missingness law: all eight clocks are not required for every security. `PRESENT/PARTIAL/MISSING/NOT_APPLICABLE` remains explicit with no imputation, neutral fill, denominator shrink, current-survivor repair or CRV1 risk-set rewrite. A later frozen `CoveragePolicyV1` may govern representation eligibility only.
- E3/E4 boundary: E3 freezes meaning/role/source-state coordinates/availability/aliasing/invariance/falsifiers/missingness. E4 must freeze deterministic `LEVEL/DELTA/INFLECTION/ORDERED_SEQUENCE` transforms and one outcome-blind sequence/lag law where needed; no H/K/lag/horizon/multi-clock outcome bake-off, no auto-L5.
- Next preferred slice=`CRV1-E4-STRUCTURED-CLOCK-REPRESENTATION-1`. W9 remains closed; future `CRV1-E1E2-RISKSET-CUSTODY-ADMIT-1` exists only after explicit shared W3/W9 raw CIQ authorization and consumes already-frozen W9 admission semantics.
- Canonical files: `docs/architecture/cycle_resonance_v1_clock_state_coordinate_preflight_v1.json`, `docs/architecture/cycle_resonance_v1_clock_state_coordinate_preflight_v1.md`, `docs/context/e2e_evidence/crv1_e3_clock_state_coordinate_preflight_20260812.json`.

## CRV1 E4 Structured Clock Representation Registry (2026-08-12)

- Slice=`CRV1-E4-STRUCTURED-CLOCK-REPRESENTATION-1`; terminal=`REPRESENTATION_CONTRACT_PASS`; outcomes/provider/returns/labels/model/ranking/L5=`0`; trial debit=`0`; `financial_alpha_evidence=0`; W9 remains closed.
- Required core representations: `SUPPLY_CAPACITY_CLOCK=LEVEL`; `INVENTORY_CLOCK=DELTA`; `PRICING_CLOCK=INFLECTION`; `UTILIZATION_MARGIN_CLOCK=INFLECTION`; `EARNINGS_REVISIONS_CLOCK=DELTA`; `EXPECTATION_GAP_CLOCK=INFLECTION`. `ORDERED_SEQUENCE` is graph-level only; there is no per-clock representation grid.
- Supply level law: favorable event is first lawful decision cut with `PRESENT` positive supply/capacity discipline. Capex direction alone cannot establish it.
- Inventory delta law: favorable direction is inventory-burden normalization under consistent period/units/scope. Numeric burden may use a lawful comparable inventory/revenue pair; denominator-only movement, write-down/reclassification, or incompatible scope cannot create positive normalization.
- Pricing/utilization-margin inflection law: `POSITIVE_TURN` requires sequential lawful states from `NEGATIVE|NEUTRAL -> POSITIVE`. Margin alone cannot create pricing state; margin-only or utilization-only evidence remains `PARTIAL` for the joint utilization/margin core clock.
- Earnings-revision delta law: among lawfully observed non-zero allowed historical-vintage EPS/revenue revision directions, all must agree to emit POSITIVE or NEGATIVE; opposing non-zero directions=`MIXED/CONTRADICTED`; all-zero=`NEUTRAL`; no lawful revision observations=`UNKNOWN`. No weights and no 30d/90d outcome-selected subset.
- Expectation-gap inflection law: favorable resolution is `UPSTREAM_AHEAD -> ALIGNED` using same-cut-compatible upstream and expectation states; market price/return/confirmation inputs are forbidden. E4 does not infer within-category magnitude.
- Sequence identity=`CRV1_CORE6_DETECTION_ORDER_MAX183D_V1`. Required order=`SUPPLY_CAPACITY -> INVENTORY -> PRICING -> UTILIZATION_MARGIN -> EARNINGS_REVISIONS -> EXPECTATION_GAP`; all five adjacent edges required; `allowed_skipped_edges=[]` for `ESTABLISHED`.
- Sequence lag formula: for every required edge `A->B`, `0 <= core_event_at(B) - core_event_at(A) <= 183 calendar days`; `core_event_at` is first lawful decision-time detection under `available_at <= cut`; same-cut is allowed. `183d` is a single approximately two-quarter mechanistic prior and is independent of the 252-trading-day outcome horizon. No lag grid.
- Sequence states: `ESTABLISHED` iff all six favorable core events are `PRESENT`, all five required edges satisfy order/lag, and no contradiction exists. Missing/PARTIAL/UNKNOWN core clock or ordered edge older than 183d => `PARTIAL` if no contradiction. Downstream-before-upstream, unfavorable predecessor reversal before downstream detection, or internally contradicted required clock => `CONTRADICTED`. Missingness alone is never negative evidence and never rewrites the risk set.
- Non-core law: `DEMAND_CLOCK` stays corroborating only and cannot repair/fill/skip the core. `MARKET_CONFIRMATION_CLOCK` stays downstream `ACTION_EXECUTION`, is excluded from core sequence status, and cannot rescue upstream economics.
- Preferred future slice=`CRV1-E5-CLAIM-INTERPRETER-CONTRACT-PREFLIGHT-1`, not auto-opened. Canonical files=`docs/architecture/cycle_resonance_v1_structured_clock_representation_v1.json`, `docs/architecture/cycle_resonance_v1_structured_clock_representation_v1.md`, `docs/context/e2e_evidence/crv1_e4_structured_clock_representation_20260812.json`.
