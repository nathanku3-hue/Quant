## 2026-07-27 One-Case Evidence-Gap Triage Commit A Registry

- Observation class: `EVIDENCE_GAP_TRIAGE_ONLY`; standing score 39, stage `CERTIFIED_MULTI_SOURCE_CASE_OPERABLE`, observed 0, no alpha claim.
- Static custody: `experiment_binding.json` binds case/cutoff, exact nine-file positive allowlist and SHA-256 values, equal maximum 3,600-second budgets, early submission, no latency endpoint, action set, rubric, projection schema, and `OPENSSH_SSHSIG_V1`; it deliberately contains no candidate SHA/tree.
- Projection custody: `projection := f(exact allowlisted bytes, frozen dimension rules)`; every read is instrumented and the complete read set must equal the allowlist. Direct, relative, symlink, junction, hard-link, alias, outside-repository, and hash-mutation paths fail closed before bytes are admitted.
- Projection boundary: includes admissible evidence/locators, overlap/non-overlap, `CLAIM_INSUFFICIENT`, dimension states, missing indispensable evidence, contradictions/falsifiers, and claim separation; excludes prior adjudication, operator confirmation, research/portfolio actions, result/certification/current-authority identities, score/stage/count, and Alpha publication output.
- Runtime candidate custody: after exact hosted Windows/Linux green, a pinned proof issuer signs `hosted_proof_identity := (candidate_sha, candidate_tree, workflow, Windows/Linux run IDs + SUCCESS conclusions, proof_id, verified_at)`, then `session_manifest_hash := H(candidate_sha, candidate_tree, experiment_binding_hash, evidence_bundle_hash, projection_hash, projection_manifest_hash, projection_schema_hash, instruction hashes, hosted_proof_identity, hosted_proof_hash, nonce, one_shot_state)`. Session-state creation replays that signature; self-asserted/tampered green JSON or artifact substitution is invalid.
- Human identity rule: operator and reviewer each provide an independently verified SSHSIG role signature plus a pinned-issuer signature linking credential to `IN_PERSON_OR_LIVE_VIDEO_GOVERNMENT_ID_MATCH` human-subject commitment. Distinct principal strings alone are insufficient; subject commitments and credentials must both differ. `BASELINE_OPEN` verifies the complete operator record and binds its hash before exposure; terminal replay rejects later operator substitution. Reviewer preflight proves viability only; final reviewer evidence additionally binds the exact review-package and rubric hashes.
- Budget rule: each arm has a maximum of 3,600 seconds; early submission is allowed; actual elapsed times may differ; `latency_endpoint = NONE` and latency improvement inference is prohibited.
- One-shot rule: `PRE_EXPOSURE_ABORT` before any evidence does not consume; `BASELINE_OPEN` irrevocably consumes; every later abort/violation/incomplete path is terminal ineligible and does not authorize a rerun.
- Blinding rule: reviewer arms retain each newly authored current research action and rationale so defensibility is scoreable, while origin metadata and all prior Alpha/portfolio answers remain excluded.
- Terminal authority rule: `REVIEW_AUTHORITY_SEAL := replay(operator signed identity, final package/rubric-bound reviewer identity, all pre-reveal event hashes, rubric, review package, detached attestation)`; only after that seal may `MAPPING_REVEAL` occur and emit an eligible non-publishing result.
- Value rule: `IMPROVED iff total_delta > 0 AND targeted_gain AND core_safety_not_worse`; every other complete eligible result is `NOT_IMPROVED`. Commit A has no publication authority; Commit B alone may import a terminal result and promote observed count/stage.
- Implementation paths: `core/gv_one_case_delta.py`, `scripts/gv_one_case_delta_capture.py`, `data/gv_one_case_delta/case_1/*`, and focused tests under `tests/gv_fs0_product/test_one_case_delta_*`.
