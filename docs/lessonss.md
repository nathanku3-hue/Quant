...
- Date: 2026-08-11
- Mistake or miss: the first AO-K0 freeze still treated insufficient feature coverage as a possible `COVERAGE_FAILURE` and gave a separately named AO denominator. That structure could make missing names disappear from the effective base rate, and it invited reuse of old assembled-feature artifacts to decide where observability begins.
- Root cause: data completeness, scientific observability and economic opportunity were conflated. A complete-case sample is useful for conditional information tests, but it is not the opportunity universe. Once missingness can shrink the denominator, an observability failure can be converted into apparent selectivity or free leverage. Separately, a historical coverage percentage can become a hidden calibration target if the source law is adjusted until it reproduces that number.
- Fix applied: bind AO-K0A directly to immutable `PREBREAKOUT_US_PRIMARY_COMMON_DATE_LOCAL_V1`; represent `Q_UNOBSERVED / M_WARMUP / M_MISSING_HISTORY / Q_AND_M_MISSING` as persistent full-W3 ABSTENTION states; freeze full-W3 right-tail/breadth accounting, zero risky weight, economic-cash residual capital, PIT-EW full-W3 opportunity comparison, and no security/peer return imputation or observed-subset renormalization. Orthogonalize M only after Q and M are independently re-ranked on the common date-local sample.
- Source guardrail: observability boundaries may use only immutable W3, admitted S0 and exact W3 market custody. Test assembly outputs, execution-microstructure fragments, transient factor stores, survivor projections and alternate-listing bridges are not observability authority. If an old coverage number cannot be reproduced from the allowed first-principles sources, downgrade the number rather than tune the source definition to match it.
- Additional guardrail: do not claim a numeric Q kernel exists merely because Q source custody exists. Here admitted S0 cannot exactly reconstruct the old Rule100 Q primitives, so AO-K0A freezes source observability and orthogonal geometry only; AO-K0B stays blocked until numeric Q and scalar Q+M-perp composition are source-bound before outcomes.
- Evidence: `research/asymmetric_opportunity_v1/orthogonalization.py`, `scripts/ao_k0a_orthogonal_basis_preflight.py`, `docs/architecture/orthogonalization_contract_v1.md`, `docs/architecture/ao_k0a_orthogonal_basis_preflight_v1.json`. Source preflight matrix=`310329` rows / SHA-256 `bd36a6305f38ff68c57f6ccfb9d3481be6fd42d2288128ef9ec3eb3cc12df5cf` / rows removed=`0`; focused deterministic suite=`7/7 PASS`; W6/outcome/provider reads=`0`; `financial_alpha_evidence=0`.
...
## 2026-08-10 Round Entry (Stop Authority Escalation, Not Negative-Knowledge Learning)

- Date: 2026-08-10
- Mistake observed: two distinct boundaries were being collapsed into one. First, `CAPTURE=NO-GO / successor trial=NO-GO / W6=HOLD` was being read as a worker stop even though Trial #1 development outcomes were already legally open and still contained material negative knowledge. Second, W4 engineering smoke accidentally reused an outcome field: `smoke_traces.any_legitimate_prebreakout_flag` required `winner_label=True`, even though MU/SNDK smoke law applies to every PIT-eligible breakout-B episode at zero weight.
- Root cause: acceptance semantics leaked into engineering-smoke semantics, and authority-promotion state was used as a proxy for information-availability state. Both errors destroy useful evidence: the first idles legal diagnostic capacity, while the second makes a zero-weight integration check outcome-contaminated.
- Fix applied: removed `winner_label` from the smoke evaluator and added a regression proving a PIT-eligible nonwinner breakout episode can still have a legitimate pre-B smoke flag. Kept the sealed Trial #1 Atlas immutable and superseded only its smoke subfield with the independent checker (`19 checked / 3 legitimate pre-B / 16 no-flag / 4 deferred`). Added `FailurePacketV1` as a read-only, zero-trial-cost stage that exhausts score monotonicity, miss taxonomy, lead shape, winner-payoff convexity, false-winner persistence/concentration, catastrophic downside, cross-A2 right-tail clipping, and frozen-manifest observability demand without model-law mutation.
- Guardrail for next time: every `NO-GO/HOLD` must name the prohibited **authority action** rather than implying a global worker stop. If outcomes are already legally open, negative-knowledge analysis may continue unless it spends search budget or changes the hypothesis. Separately, zero-weight smoke/integration probes must be outcome-label independent by construction; acceptance labels may never enter their eligibility predicate. When a sealed evidence object contains a non-closing diagnostic defect, preserve immutable bytes and issue a narrowly scoped superseding truth rather than rewriting history.
- Evidence paths: `research/prebreakout_atlas_v1/atlas.py`, `tests/prebreakout_atlas_v1/test_atlas.py`, `research/prebreakout_discovery_v1/failure_packet_v1.py`, `scripts/prebreakout_failure_packet_v1.py`, `tests/prebreakout_discovery_v1/test_failure_packet_v1.py`, `docs/architecture/prebreakout_failure_packet_v1.md`, `docs/context/e2e_evidence/prebreakout_failure_packet_v1_20260810.json`. Focused regression=`22/22 PASS`; `financial_alpha_evidence=0`; W6 untouched.

## 2026-08-10 Round Entry (Probe State Change Cheaply; Spend Full Fundamental Requests Only on New Information)

- Date: 2026-08-10
- Mistake avoided: a historical-PIT structured-state slice can become expensive and subtly hindsight-conditioned if it builds a union master and queries every entity on every weekly date, or if it pulls a full five-quarter fundamental panel on the first observed date before any state transition is known.
- Root cause: confusing a successor-wide identity master with the date-local W3 eligible risk set, and confusing baseline establishment with information arrival. Those shortcuts over-request entities outside the legitimate date-local state and spend provider calls that add no transition information.
- Fix applied: freeze a one-to-one successor master for identity, but drive period probing from exact **date-local W3 eligible entity-date pairs** only. Query only `IQ_PERIOD_END/FQ0` first. Treat the first nonmissing FQ0 as baseline, and issue five-quarter Original fundamentals only after an adjacent nonmissing FQ0 period change. A missing probe breaks adjacency; never bridge over it. This reduced the real pre-W6 request to `310,329` cheap probes over `64` weekly dates and preserves explicit missingness.
- Guardrail for next time: **identity scope and request scope are different contracts.** A union master may validate identity, but historical acquisition must stay date-local. Separate cheap change detection from expensive payload capture, and define baseline/missing adjacency before provider access. Never infer a transition across an unobserved probe merely to recover coverage.
- Evidence paths: `scripts/econphysics_prebreakout_s0_request.py`, `scripts/aov0_capture_ciq_historical_pit_productquery.py`, `docs/architecture/econphysics_prebreakout_s0_structured_request_v1.json`, `data/prebreakout/compiled/econphysics_s0_request_20260810/`. S0 focused=`11/11 PASS`; legacy ProductQuery+S0=`15/15 PASS`; no provider capture executed.

## 2026-08-10 Round Entry (All-or-Nothing Provider Capture Must Fit the Execution Envelope or Be Restartable Before GO)

- Date: 2026-08-10
- Mistake or miss: the frozen S0 request correctly minimized provider semantics but still treated `310,329` FQ0 probes as one all-or-nothing in-memory capture. Under the available DevSpace execution host, commands are hard-capped at `300s`; the first real authorized run completed `53,200` exact requests successfully but was terminated before the writer could atomically land any raw bytes.
- Root cause: semantic minimization and operational restartability were frozen separately. The provider request was narrow, but the transport contract did not bind a bounded chunk size/checkpoint identity even though the execution environment has a known hard runtime ceiling. Atomic-at-end therefore converted successful provider work into unrecoverable non-custody and made a naive retry a duplicate rescue query.
- Fix applied: stop immediately after the timeout; verify no raw CSV/receipt/partial admission exists; do not restart, change batch size, chunk retrospectively, backfill or rescue under the user's explicit no-rescue boundary. Record the attempt as `OPERATIONAL_TRANSPORT_CAPACITY_BLOCKER`, not provider-semantic or mechanism failure.
- Guardrail for next time: **before provider GO, the frozen request must prove that either the whole capture fits the host execution envelope or every provider batch/chunk is atomically hash-bound and resumable without duplicate queries.** Transport chunking may not be improvised after partial provider access; it must be frozen ex ante with exact chunk membership, receipts, merge law and no-overlap/no-gap verification.
- Evidence paths: `docs/context/e2e_evidence/econphysics_prebreakout_s0_capture_attempt_20260810.json`, `docs/architecture/econphysics_prebreakout_s0_structured_request_v1.json`, `scripts/aov0_capture_ciq_historical_pit_productquery.py`. Completed provider requests=`53,200`; admitted rows=`0`; rescue queries=`0`; `financial_alpha_evidence=0`.

## 2026-08-11 Round Entry (Do Not Confuse Representation Failure with Observable Insufficiency)

- Date: 2026-08-11
- Mistake caught: the S0 v2 `NO_EXTRACTION_LIFT` result was being interpreted as “structured fundamentals do not add transition information,” even though both M0 and M1 shared the same persistence direction law. That conclusion collapsed two hypotheses: whether the representation contains information and whether the chosen state→next-transition operator has the correct dynamics.
- Root cause: the evaluator correctly tested the frozen persistence hypothesis, but the interpretation layer treated failure of that operator as failure of the observable surface. Inventory and margin quarterly dynamics are materially mean-reverting in the admitted corpus, so a shared wrong operator can make both a coarse and a richer representation fail together.
- Fix applied: add a diagnostic-only, fixed low-freedom dynamics family on the identical PIT corpus/folds/XS holdout: level persistence, delta persistence, delta mean reversion, delta2 acceleration, plus mean-reverting application of the already-frozen M0/M1 state representations. Inventory survives; margin survives through reversed M1; revenue does not. The shootout interpretation contract now routes `NO_EXTRACTION_LIFT` to operator diagnosis before observable insufficiency.
- Guardrail for next time: **a failed representation under a shared transition operator cannot establish observable insufficiency.** First separate `representation × dynamics operator`. Route node by node. Only after the complete low-freedom operator family fails with adequate coverage may that node escalate to new upstream observables. Keep causal observables (orders/backlog, pricing/mix, utilization/capacity, channel inventory) separate from expectation observables (revisions/guidance/consensus).
- Evidence paths: `research/econphysics_prebreakout_v1/dynamics_diagnostic.py`, `scripts/econphysics_prebreakout_s0_dynamics_diagnostic.py`, `data/prebreakout/analysis/econphysics_s0_economic_dynamics_diagnostic_v2.json`, `docs/context/e2e_evidence/econphysics_prebreakout_s0_dynamics_diagnostic_20260811.json`. Same-corpus posterior diagnostic only; `financial_alpha_evidence=0`; W6 untouched.

## 2026-08-11 Round Entry (Freeze the Scientific Unit Before Expanding the World Model)

- Date: 2026-08-11
- Mistake avoided: after discovering node-specific inventory/margin dynamics, the obvious continuation was to deepen the econphysics causal chain into more orders/pricing/capacity/expectation data. That would have treated one promising mechanistic branch as the universal stock-discovery ontology before proving that simpler quality and market-recognition bases carry independent payoff information.
- Root cause: mechanism depth is easy to confuse with scientific progress. A richer company world model can absorb research time and search freedom without answering the more basic question: what is the smallest PIT representation that increases favorable forward-payoff density without purchasing unacceptable downside?
