## 2026-07-27 Round Entry (Hosted Green Requires Durable Detached Evidence)
- Date: 2026-07-27
- Mistake or miss: hosted Product and Protocol runs for candidate `14b3773` were observed through the GitHub API, but the candidate branch still contained a phase brief and SAW that said hosted proof was pending; the transient observation was then described as a completed stop gate. Root-checkout dirtiness was also at risk of being conflated with candidate-worktree custody.
- Root cause: run status observation, durable proof receipt, and candidate commit identity were treated as one object even though hosted proof necessarily occurs after the candidate commit. The local root checkout and isolated candidate worktree were also discussed without an explicit custody boundary.
- Fix applied: retained `14b3773` as the immutable runtime candidate; created a separate hosted-receipt evidence branch; recorded exact candidate SHA/tree, Product and Protocol run/job IDs, Windows/Linux/parity conclusions, and verification time in an `OPENSSH_SSHSIG_V1` receipt; pinned the proof-issuer public key; validated the receipt through `verify_hosted_proof()`; added focused receipt tests; and stated that root `main@accef5c` is a separate unsafe checkout, never candidate evidence.
- Guardrail for next time: never infer durable hosted-green custody from a transient API query or from prose written before the run completes. Every post-commit hosted gate needs a detached, replay-verifiable receipt bound to the prior candidate; candidate worktree and root checkout status must always be reported separately.
- Evidence paths: `data/gv_one_case_delta/hosted_receipts/14b3773/hosted_proof.json`, `data/gv_one_case_delta/hosted_receipts/14b3773/trusted_proof_issuers.json`, `tests/gv_fs0_product/test_one_case_delta_hosted_receipt.py`, `docs/phase_brief/gv-one-case-decision-delta-brief.md`, and `docs/saw_reports/saw_gv_one_case_delta_commit_a_20260727.md`.

## 2026-07-27 Round Entry (Human Value Tests Need Non-Self-Referential Custody and Positive Input Control)
- Date: 2026-07-27
- Mistake or miss: the proposed one-case comparison treated the shipped Alpha export as the post-arm packet, required only packet naivete, embedded GitHub identity fields as protocol semantics, attempted to bind a commit SHA inside a file contained by that same commit, relied on forbidden-output scanning without proving which source files were read, removed the current-arm action needed to score defensibility, and left one-shot timing/abort semantics ambiguous.
- Root cause: custody-complete product output was mistaken for an unbiased decision-support input; account inequality was mistaken for proof of two humans; static experiment custody and runtime candidate custody were conflated; output sanitation was mistaken for source-custody enforcement; blinding was over-applied to the operator's newly authored answer; and equal time budgets were conflated with equal elapsed time or a latency endpoint.
- Fix applied: classified the run as `EVIDENCE_GAP_TRIAGE_ONLY`; prohibited prior Alpha adjudication, action, result, operator, portfolio, and certification identities; added a positive path-and-hash source allowlist plus forbidden-path access tests; moved candidate binding into post-hosted-green `session_manifest.json`; selected provider-neutral `OPENSSH_SSHSIG_V1` with separately signed issuer and role challenges for distinct verified-human commitments; retained each blinded arm's current research action and rationale; defined equal maximum 60-minute budgets with early submission and no latency inference; and separated pre-exposure abort from a consumed terminal run.
- Implementation misses found by tests, authority review, and hosted parity: converting elapsed duration to integer seconds before enforcing the limit admitted a submission one microsecond beyond 60 minutes; Windows text-mode stdin rewrote canonical SSHSIG message bytes; the direct CLI lacked repository-root import bootstrap; reviewer identity did not yet bind the exact review package and rubric; terminal eligibility accepted caller-supplied identity/attestation hash strings; session creation accepted a locally editable JSON object asserting hosted Windows/Linux green; exposure state initially did not bind the exact bundle/projection bytes or verify the complete operator identity record before `BASELINE_OPEN`; and the first hosted Windows checkout converted newly tracked canonical JSON from LF to CRLF because the new data subtree lacked an explicit `.gitattributes` rule.
- Implementation correction: compare exact duration before integer reporting; verify canonical SSHSIG bytes through binary stdin; bootstrap the CLI repository root; distinguish reviewer preflight evidence from final package/rubric-bound reviewer evidence; require `seal_review_authority()` to replay both signed identities, complete event hashes, rubric, package, and detached attestation before mapping reveal; require a pinned-provider signed hosted-proof receipt binding the exact candidate SHA/tree and both successful matrix runs; bind the session to exact bundle/projection/manifest/schema hashes; verify the full signed operator identity before exposure; reject post-exposure operator substitution; and pin all one-case JSON/instruction files to LF with a checkout-custody regression. Focused boundary/signature/CLI/hosted-proof/artifact-binding/terminal-authority tests pass locally; hosted repair parity remains mandatory before exposure.
- Guardrail for next time: never place a containing commit's SHA inside its own committed binding file; prove both what a projection may read and what it may emit; treat signatures as credential control unless independently linked to verified human subjects; enforce time boundaries before lossy conversion; treat canonical signature messages as bytes across platforms; pin canonical data subtrees to LF before the first cross-platform push; retain the arm answer needed by the rubric while removing only prior answers and origin metadata; and define the exact event that consumes a one-shot authorization before any human exposure.
- Evidence paths: `core/gv_one_case_delta.py`, `tests/gv_fs0_product/test_one_case_delta_projection.py`, `tests/gv_fs0_product/test_one_case_delta_capture.py`, `tests/gv_fs0_product/test_one_case_delta_attestation.py`, `docs/phase_brief/gv-one-case-decision-delta-brief.md`, `data/gv_one_case_delta/case_1/projection_manifest.json`, and `docs/architecture/godview_endgame_vision.md`.
