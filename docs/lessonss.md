## 2026-08-04 Round Entry (Economic Evidence Before Integration and UI)
- Date: 2026-08-04
- Mistake or miss: the first execution plan placed Command Center integration before the cascade four-arm experiment and treated the evidence card beside the receipt as if both were critical-path product work. It also left combined-arm mathematics and the experiment exit behavior underspecified.
- Root cause: product-surface readiness was coupled to economic falsification, and two independent risk controls were assumed composable without freezing whether their scalars multiply or compete as caps. Expiry was treated as enough despite sold positions requiring an explicit terminal disposition.
- Fix applied: banked the cascade candidate first; reused the existing G5 nonzero canonical portfolio; ran A/B/C/D without Command Center integration; froze `D_cap=min(uncapped, regime, cascade)`; emitted an ex-ante receipt; bound horizon, max holding, review date, restore/remain-reduced choice, and reconciliation date; labelled the synthetic Leningrad result `ENGINEERING_ONLY`; retained an honest negative/neutral D-versus-B result. A managed-worktree allocator created empty directories, so execution continued from the clean pushed checkout while preserving the banked parent commit.
- Guardrail for next time: keep UI and merge work off the pre-evidence path; make cross-control mathematics explicit before a run; require a complete exit rule before confirmation; negative experiments are shipped evidence, not reasons to rerun for a preferred sign; verify managed worktrees are actual Git checkouts before relying on them.
- Evidence paths: `research/financial_cascade_four_arm.py`, `scripts/run_financial_cascade_four_arm.py`, `tests/test_financial_cascade_four_arm.py`, `docs/context/e2e_evidence/gv_financial_cascade_four_arm_engineering_20260804.json`, and `docs/phase_brief/gv-financial-cascade-shadow-0-brief.md`.
...
## 2026-08-01 Round Entry (A Scale Spike Must Test the Product Path, Not Only the Domain Core)

- Date: 2026-08-01
- Mistake or miss: initial transient-registry and in-memory probes made the 50/100 domain flow look operationally bounded even though the real product path could not persist either scenario. The 100 run also exposed timestamp strings that remained hash-deterministic while being invalid ISO timestamps.
- Root cause: scenario registry generality, engine generality, persistence generality, UI operability, and data validity were initially treated as one scaling property. Determinism was also treated as close to validity even when deterministic bytes encoded malformed time values.
- Fix applied: moved only diagnostic scenarios into the existing declarative registry; added one fresh-process characterization harness; probed unchanged persistence; parsed every timestamp-like field; separated in-memory engine completion from product-path acceptance; froze diagnostic candidate `f9d271d`; stopped without repairing shared storage or timestamp behavior; and recorded SAW BLOCK instead of inferring independent review from local checks.
- Guardrail for next time: every scale claim must independently test engine execution, timestamp/data validity, persistence/save/reopen, product operator flow, and fresh-process equality. A deterministic failure is still a failure. Required redesign ends a spike with a finding and must not be smuggled into the same round. When reviewer agents are unavailable, freeze evidence honestly with SAW BLOCK and never relabel local self-review as independent closure.
- Evidence paths: `scripts/characterize_gv_engine_scale.py`, `tests/gv_portfolio_v0/test_engine_scale_characterization.py`, `docs/context/e2e_evidence/gv_engine_scale_characterization_1_20260801.md`, `docs/context/gv_p1_custody_model_decision.md`.

## 2026-08-02 Round Entry (Runtime Fixtures Prove Capability, Not Prospective Evidence)

- Date: 2026-08-02
- Mistake or miss: an initial prospective plan risked treating test-injected runtime observations as prospective evidence, allowed `CASH` to appear conceptually beside instrument outcomes, and proposed extending the fixed scenario-authored status/count validator for repeated episodes.
- Root cause: runtime configurability, operator authority, and real prospective evidence were conflated. The existing fixture-driven engine encouraged adding more authored branches instead of introducing one event-projected runtime authority path.
- Fix applied: derived one prospective profile from the accepted 25-security catalogue; introduced an explicit runtime observation and review-proposal envelope; kept score, quantity, thesis, and outcome non-authoritative until mutation-free preview validation and confirmation; constrained instrument outcomes to `ADMIT/REJECT/ABSTAIN`, kept `CASH` portfolio-level, required non-`ADMIT` target quantity `0`, and reconstructed confirmed and rejected episodes through one append-only projector.
- Guardrail for next time: call evidence prospective only when a human supplies previously unauthored content during actual operation. Automated runtime fixtures may certify interfaces and determinism, never the prospective endpoint. Every proposed decision field must name its confirmation boundary, and repeated state must project from append-only authority rather than enumerated scenario statuses.
- Evidence paths: `gv_portfolio_v0/prospective.py`, `gv_portfolio_v0/operated_scenarios.py`, `gv_portfolio_v0/operated_storage.py`, `views/gv_prospective_paper_workspace.py`, `tests/gv_portfolio_v0/test_prospective.py`, `tests/gv_portfolio_v0/test_prospective_app.py`, `docs/phase_brief/gv-prospective-paper-baseline-1-brief.md`.

## 2026-08-03 Round Entry (A Generic Class Name Does Not Make a Single-Case Workflow Portfolio-Wide)

- Date: 2026-08-03
- Mistake or miss: successive designs used generic `CapitalProposal` and lifecycle names while the actual acceptance flow still asked whether to stage one MU proposal; later designs also tried to compose multiple absolute targets by summing independently calculated deltas.
- Root cause: the first real case, the workflow unit, UI pages, proposal lifecycle, and portfolio authority were repeatedly conflated. Generic naming hid MU-centric behavior, and unit normalization was mistaken for target-intent resolution.
- Fix applied: made `DecisionEpisode` the portfolio-wide PIT consensus unit; made pages read projections; separated immutable proposal evidence from event-backed lifecycle; separated preview from authorization; introduced explicit `TARGET_FINAL`/`DELTA`/`OVERLAY` intent; restricted initial composition to zero/one base target plus compatible overlays; retained `RISK_BPS` outside neutral notional conversion.
- Guardrail for next time: prove generality by running multiple competing proposals against one certified book and one operator decision, not by creating generic schemas around one case. Never aggregate targets until intent, unit, pricing, sleeve, and conflict policy are explicit.
- Evidence paths: `docs/architecture/dashboard_all_capital_pit_contract.md`, `docs/architecture/top_level_roadmap.md`, `docs/phase_brief/gv-dashboard-all-capital-pit-1-brief.md`.

## 2026-08-03 Round Entry (Event-Sourced Labels Are Not Event Authority)

- Date: 2026-08-03
- Mistake or miss: the planning sequence called proposal and episode objects event-derived after adding event class names, but still lacked a command authority boundary, ordered stream envelope, append/read contract, digest chain, and a rejection path that could always reconstruct the full rejected proposal. The shell-first file scope also assumed production JSON/Parquet and cash artifacts that were not grounded in the checkout.
- Root cause: immutable dataclasses, event vocabulary, persistence, and actual event authority were treated as equivalent. Architectural generality was inferred from names rather than from who decides facts, how facts are ordered, and whether the real repository objects traverse the complete product path.
- Fix applied: added a typed command-handler boundary for five-field identity decisions; required complete proposal custody on accepted and rejected paths; froze stream/sequence/correlation/causation/digest identities and duplicate/gap rules; restricted the projector to a pure fold and Slice 1 to its actual event-backed states; grounded adapters in the prospective workspace, independent shadow object, and certified-book cash; replaced the seven-file shell-first scope with contracts → adapters → handler → events → projector → read-only Command Center as one product transaction.
- Guardrail for next time: before calling a model event-sourced, name the command that requests change, the component that decides, the complete facts emitted, the ordering/idempotence/digest rules, the authoritative append/read seam, and the pure replay function. Before naming an adapter or deletion target, prove the current repository source/path and fail closed on missing economic identity rather than inventing a file or value.
- Evidence paths: `docs/architecture/dashboard_all_capital_pit_contract.md`, `docs/architecture/dashboard_all_capital_pit_planning_checklist.md`, `docs/architecture/top_level_roadmap.md`, `docs/phase_brief/gv-dashboard-all-capital-pit-1-brief.md`, `docs/context/done_checklist_current.md`.
