...
> **2026-08-10 ECONPHYSICS × WINNER_SELECTION recut:** The original market-pattern discovery framing in this memo is superseded for future PREBREAKOUT mechanism design by `docs/architecture/econphysics_winner_selection_integration_lock_20260810.md`. Winner selection remains required, but its state representation must be generated by ex-ante causal economic physics + expectation gap. Top-5% winners, Recall/Lift, TTFLD, W4/W5 decompositions, and MU/SNDK may falsify/measure the integrated system but may not invent causal variables, edges, signs, lags, or thresholds. `PREBREAKOUT_TRIAL1_M0_MARKET_EARLY_WARNING_V1` remains a legitimate preregistered `FAIL`, permanently charged `1/8`, with W6 untouched. The successor `ECONPHYSICS_PREBREAKOUT_v1` causal contract + PIT observable manifest are now frozen at `docs/architecture/econphysics_prebreakout_v1_causal_contract.md` and `docs/architecture/econphysics_prebreakout_v1_pit_observable_manifest.json`; **this satisfies the methodology prerequisite but does not authorize capture, an old-family Trial #2, a successor empirical trial/prediction clock, or W6 today.**
...
> **Freeze causal economic physics before collecting new result-bearing observations. Use that state representation to rank the full PIT cross-section; let winner/right-tail outcomes falsify selection quality, not design the mechanism; then let prospective time, not another adaptive historical-pattern loop, decide promotion.**
