...
Authority is deliberately two-layered:

- this consolidated contract defines the normative V1 semantics; and
- the 18 generated artifacts are the machine-readable normative expression of those semantics.

The generator is only a deterministic derivation mechanism. It is not a third protocol authority. It may contain no protocol constant absent from this contract or from a named generated table, registry, schema, or vector definition. Any disagreement among this contract, generated artifacts, canonical vectors, or generator output fails the freeze.
...
Comparison uses ascending integer order, ascending ASCII bytes for `source_intent_id`, and ascending integer slot order. Duplicate origin-order keys within a group are invalid and deterministically produce `DUPLICATE_ORIGIN_ORDER_KEY`, bound to `timestamp_causality_valid = FALSE`.
...
For `NO_POSITION`, the projected source-intent array contains zero `EXECUTION_INTENT` entries. This is valid and normative; an empty execution-intent subset must not be treated as a schema or projection violation. Explicit valuation instructions remain required.
...
| `price_freshness_valid` | exactly one positive price exists for the same security and valuation session, `price_timestamp <= valuation_timestamp`, and session lag equals zero | missing, duplicate, cross-session, future-timestamped, non-positive, mismatched, or unordered price | price validation could not execute |
...
The V1 registry must include `DUPLICATE_ORIGIN_ORDER_KEY` with category `CAUSALITY`, terminal outcome, applicability to `timestamp_causality_valid`, outcome `FALSE`, and emitters `PRIMARY` and `VERIFIER`.
...
After the approved freeze candidate lands, V1 bytes are immutable. Any correction to a frozen V1 surface, including a purportedly editorial correction, requires a new protocol version and re-audit.

Canonical byte fields in vectors use lowercase hexadecimal. SHA-256 fields use lowercase 64-character hexadecimal. `terminal_newline_count` is the number of consecutive LF bytes at the end of a file and must equal `1`. `git_blob_oid` is produced using the repository's declared Git object format, and the freeze manifest records `git_object_format` explicitly as `sha1` or `sha256`.

Vector assertions must not depend on a JSON parser reinterpreting embedded control characters. Asserted document and preimage bytes are represented as lowercase hexadecimal.
...
Initial-freeze CI uses two explicit modes:

- `BOOTSTRAP`: before V1 exists on the protected base, prove deterministic regeneration, manifest correctness, exact vector equality, no extra or missing artifacts, mutation rejection against a test-controlled frozen baseline, and contract-to-artifact version consistency.
- `ENFORCED`: after V1 lands on the protected base, compare every frozen V1 surface against that base. Before reducer authorization, a non-mergeable mutation-probe branch must demonstrate rejection of a one-byte schema change, registry semantic change, contract change, vector change, CRLF conversion, and an artifact plus dishonestly updated manifest changed together.
...
- duplicate origin-order key rejection with registered `DUPLICATE_ORIGIN_ORDER_KEY` causality binding;
...
- NO_POSITION accepts zero execution intents and still requires valuation intents;
- exact zero-session-lag price freshness and future-price rejection;
...
5. bootstrap CI passes, the frozen-base mutation probe fails for the expected reasons, and enforced-mode protection is ready for the protected base; and
