...
AO-K0A itself consumes zero new material trials. Numeric Q remains unbound until `QSourceContractV1 ∈ {Q_GF_BOUND, Q_AMENDED_BOUND}` (current path=`Q_SOURCE_BLOCKED`). The active **research shadow** is **OK-SBI-0 @ S0** (design-locked, release-blocked, `runnable_evaluation=false`) — not a product tip and not a fourth evidence clock. Legacy `AO-K0B` is a blocked pointer, not an automatic `Q / M_perp / Q+M_perp` trophy next. Result-bearing OK-SBI development (AO-K0B-D), if authorized, requires Q bind + numeric gates + development-input/label hashes + one-shot carve-out, with science order missingness → raw joint law → Residual-M → deployability. Current hard boundary: **no empirical basis result, provider capture, K tuning, dislocation backtest, peer-valuation acquisition, portfolio optimization, W6 access, Parent/Child/A2/Clock #1 mutation, or capital authority from AO-K0A / OK-SBI S0.** `financial_alpha_evidence=0`. Docs mode for this sync=`DELTA_CURRENT_TRUTH_ONLY` (2026-08-12).
...
Q = quality
M = multi-speed trend
F = dimensionless capital-pressure proxy
C = accumulated pressure / proxy crowding
L = bounded exit capacity
R = regime
U = model/data uncertainty
```
...
Historical evidence is preserved; historical runtime compatibility is not.

Current AOV authority has:
