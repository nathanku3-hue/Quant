...
def _render_strategy_replay_section(full_context: DashboardReplayContext) -> None:
    ...
    # ── ENTER/EXIT Events (same bundle identity as daily replay rows) ──
    ...
    if not event_df.empty:
        st.markdown(f"**ENTER/EXIT Events** *(replay: {full_context.method}; source: {full_context.source_id or full_context.source_mode})*")
        _render_event_ledger_chart(event_df, list(event_df["ticker"].unique()))
    else:
        st.info("No trade events in this replay window.")

    # ── Buy/Sell Decision Log (same bundle.decision_rows source) ──
    ...
    if not decision_df.empty:
        ...
        with st.expander(f"**Buy/Sell Decision Log** ({len(decision_df)} rows)", expanded=False):
...
def _render_event_ledger_chart(df_filtered: pd.DataFrame, selected_tickers: list) -> None:
    """Render ENTER/EXIT markers from the event ledger."""
    fig = go.Figure()
...
page = build_dashboard_navigation(
    {
        ...
        "Discovery & Analysis": _render_discovery_page,
        "Entry/Exit Strategy": _render_strategy_page,
    }
)
page.run()
