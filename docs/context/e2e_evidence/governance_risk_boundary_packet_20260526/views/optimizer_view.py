...
def render_optimizer_view(
    ...
    if (
        ...
        if not hold_weights.empty and float(hold_weights.sum()) > 0:
            st.info("Current-hold replay output: no new PIT ENTER candidates today, so the page is showing open lifecycle holdings with residual cash instead of optimizer output.")
