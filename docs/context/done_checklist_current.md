...
## AO-K0A Denominator Alignment + Orthogonal Basis Preflight — 2026-08-11

- [x] Active contract=`docs/architecture/orthogonalization_contract_v1.md`; machine freeze=`docs/architecture/ao_k0a_orthogonal_basis_preflight_v1.json`; historical AO-K0 remains immutable but is superseded where it conflicts.
- [x] Denominator is exact `PREBREAKOUT_US_PRIMARY_COMMON_DATE_LOCAL_V1`; the separate AO denominator identity and all complete-case denominator rewrites are retired for the active basis.
- [x] Missingness is persistent full-W3 state: `ELIGIBLE_COMPLETE / Q_UNOBSERVED / M_WARMUP / M_MISSING_HISTORY / Q_AND_M_MISSING`. No coverage PASS/FAIL gate exists. ABSTENTION=`selected=false / risky_weight=0 / never removed from denominator`.
- [x] Q rank denominator=all Q-observed W3 names. Q and M are independently re-ranked on Q∩M before within-date `OLS rank(M) ~ 1 + rank(Q)`; `M_perp` is the residual. No temporal fitting or outcome input.
- [x] Q arm keeps Q-only observability; M-perp and Q+M-perp require Q∩M. Q is not artificially intersected down for comparison convenience.
- [x] Full-W3 evaluation law is frozen: right-tail denominator=all matured W3 winners; breadth denominator=all W3 names; abstain winners=foregone upside; abstain bottom-tail=avoided catastrophe. Security/peer return imputation and observed-subset renormalization are forbidden.
- [x] Capital mapping law is frozen without optimization: ABSTENTION risky weight=0; residual capital=canonical economic cash; opportunity comparator=PIT equal-weight full W3.
- [x] Observability boundaries use only immutable W3 + admitted S0 + exact W3 market custody. Test assemblies/transient feature stores/execution-microstructure fragments are forbidden sources.
- [x] Source-only preflight generated/content-addressed 310,329 pre-W6 weekly rows at SHA-256 `bd36a6305f38ff68c57f6ccfb9d3481be6fd42d2288128ef9ec3eb3cc12df5cf`; missingness removed `0` rows; W6/outcome/provider reads=`0`.
- [x] Historical ~79.49% common coverage is not recertified or gating authority. AO-K0A does not tune a source definition to recover it and does not borrow old Rule100 feature artifacts; numeric Q remains explicitly source-unbound for AO-K0B.
...
- [ ] Next empirical action is separately gated: source-bind numeric Q and scalar Q+M-perp composition, then hash-bind exact development inputs + uninspected labels and run one charged full-W3 `Q / M_perp / Q+M_perp` development package. Only a lawful survivor may later approach W6.
...
- [x] W4 **statistical** Atlas close is complete / sealed / fresh-process verified: `2,381` statistical winners, `909` detected, `1,472` missed, median effective TTFLD (miss=0)=`0`, detected-only median=`11`, false-winner decision rows=`90,954`, exact-control unmatched cases=`0`. The sealed Atlas bytes were not regenerated.
- [x] Material-but-non-closing smoke defect isolated and repaired in the evaluator: `smoke_traces.any_legitimate_prebreakout_flag` no longer depends on `winner_label`. Independent MU/SNDK development smoke truth=`19 checked / 3 legitimate pre-B / 16 no-flag / 4 deferred`; zero statistical/promotion weight is preserved. The old sealed Atlas smoke subfield is superseded only as smoke acceptance truth.
- [x] `pit_or_custody_invalidation=false`; Trial #1 is a valid scientific failure of the market-behavior discovery branch, not an infrastructure/custody failure. FailurePacketV1 shows the failure is not threshold-rescuable: `280,198` matured OOS rows, trigger winner rate `3.9365%` vs base `5.7120%`, every month below base, all five score quintiles below base and highest score quintile worst.
- [x] Miss taxonomy is complete and exhaustive over `1,472` missed winners: `63` no READY history, `526` never near-high, `178` no compression, `50` no volume, `610` component-present-but-not-synchronized, `45` same-row component-positive but no legal trigger. Pure READY-history coverage failure is only `4.28%`.
- [x] Convexity/downside diagnosis is frozen: payoff-quartile detection=`43.79% / 45.04% / 36.30% / 27.56%`; detected/missed winner 20d medians=`44.49% / 51.54%`; triggered nonwinner date-local bottom-5% rate=`2.27%` vs ordinary=`5.56%` (`0.408×`). Role split=`DISCOVERY=FAIL / DEFENSIVE_QUALITY=DIAGNOSTIC_POSITIVE`; defensive information is not acceptance or untouched evidence.
- [x] Cross-A2 right-tail clipping audit is diagnostic only: the A2 Child risk-improvement/return-dilution signature and Trial #1 anti-convex capture are directionally consistent with clean-state bias, but do not constitute system proof or tuning authority.
- [x] Real S0 structured PIT custody now exists locally: `23,047` admitted snapshots / `5,382` securities / `17,662` predecessor-gated transition pairs. The earlier incomplete all-or-nothing attempt remains historical evidence only; the current raw receipt uses restartable atomic provider-batch shards.
- [x] M0/M1 v2 correctly fail the frozen persistence hypothesis. That result no longer routes directly to “structured fundamentals lack information”: the 2026-08-11 diagnostic-only low-freedom operator family retains inventory via raw-Δ/M0/M1 mean reversion and operating margin via M1-state mean reversion; revenue has no `>=3/4` survivor. Global `OBSERVABLE_INSUFFICIENCY` is therefore not established.
- [x] Causal/expectation layers are separated: orders/backlog, pricing/mix, utilization/capacity and channel inventory are upstream economic observables; revisions/guidance/consensus remain downstream expectation-gap observables. The dynamics diagnostic mixes neither winner labels nor expectation inputs.
- [x] W6 remains untouched: `w6_lockbox_opened=false`, `w6_labels_opened=false`; old-family Trial #2 remains forbidden.
- [x] The 2026-08-10 mechanistic successor identity was `ECONPHYSICS_PREBREAKOUT_v1`; its causal contract + PIT observable manifest remain immutable historical authority. AO-K0 now prospectively classifies it as a specialized `FUNDAMENTAL_TRANSITION / MECHANISTIC` kernel rather than the universal stock-discovery identity. CRV1 252d outcome/risk-set/search/prediction authority remains isolated.
- [x] The historical econphysics M0 selection law remains immutable for its own branch. AO-K0A separately freezes full-W3 denominator/ABSTENTION semantics before any basis label read; no complete-case coverage gate or old SelectionBudget default may redefine the opportunity universe.
- [x] `FailurePacketV1` negative knowledge is retained at trial cost `0` and is not to be extended further; it may not mutate causal edge/sign/lag/threshold, `SelectionBudgetV1`, Parent/Child, or the successor model.
- [x] `ECONPHYSICS S0 — STRUCTURED STATE-TRANSITION PROOF` pre-capture request is frozen at `docs/architecture/econphysics_prebreakout_s0_structured_request_v1.json`: exact W3-derived `CIQSEC↔company` master=`5,733`; pre-W6 weekly as-of dates=`64` ending `2026-06-10`; exact date-local W3-eligible FQ0 probe pairs=`310,329`; W6 decisions=`20` and maturity-tail sessions=`20` excluded.
- [x] S0 ProductQuery path is narrow and backward-compatible: stage 1=`IQ_PERIOD_END/FQ0` exact entity-date probes with explicit missingness; stage 2=adjacent nonmissing FQ0 period changes only, first probe baseline-only, no bridge across missing probes; stage 3=five quarters of exactly `IQ_PERIOD_END / IQ_TOTAL_REV / IQ_INVENTORY / IQ_OPER_INC / IQ_CAPEX_BNK`, `FilingVer=Original`.
- [x] S0 runtime/evaluator is implemented under `research/econphysics_prebreakout_v1/`: frozen latest-vs-prior + YoY directions only; explicit `MIXED/UNOBSERVED`; operating-margin and inventory/revenue ratios require same validated period/unit with positive revenue; capex is cycle evidence only, never capacity-state truth; evaluator reads no market/winner/W6/selection outcome surface.
- [x] S0 mechanism gate is encoded without tuning: four temporal folds + deterministic 20% CIQSEC hash holdout; target reports include `N / base rate / directional hit / lift vs no-information baseline / contradiction / directional association / missingness`; core target support requires `lift>1` and positive directional association in a majority (`>=3/4`) of informative folds; inadequate coverage=`UNOBSERVED`, otherwise failure=`FAILED`.
- [x] S0 focused regression=`11/11 PASS`; adjacent legacy historical ProductQuery + S0 regression=`15/15 PASS`; selected compile and scoped Git whitespace checks PASS.
- [x] `CAPTURE GO / S0 ONLY` was explicitly granted and exercised without widening authority. Frozen hashes and the existing authenticated CIQ page reverified before provider access; sign-in/sign-out were not performed.
- [x] Stage-1 attempt used the exact frozen `IQ_PERIOD_END/FQ0` pair plan and batch size `200`. `266` completed batches returned exact `200/200` response counts with no provider error observed (`53,200` requests total) before the execution host enforced its `300s` hard timeout.
- [x] Fail-closed custody held: no `fq0_period_matrix.csv`, no receipt, no partial raw admission, no period-change plan, no five-quarter fundamentals request, no PIT admission and no state-transition evaluation exists from the incomplete attempt.
- [x] User `no rescue query` boundary is binding: no restart/requery, larger batch, chunking, backfill, alternate metric or rescue request was performed after the timeout. Successor empirical trial/clock, winner selection, W6, VSB, A2/CRV1 outcomes and capital remain closed; `financial_alpha_evidence=0`.
- [ ] FailurePacketV1 mandatory independent Reviewer A/B/C SAW closure is unavailable on the current tool surface; `docs/context/saw_prebreakout_failure_packet_v1_20260810.md` remains `BLOCK`. This review-capacity gap does not authorize escalation and does not require stopping legal development-only diagnosis.
...
- [x] The transport-capacity blocker was subsequently superseded by restartable local custody; no provider acquisition is required for the current dynamics diagnosis.
- [x] The former node-specific-continuation next action is superseded by AO-K0. Inventory + margin remain retained diagnostic candidates, but no additional transform is the current universal next slice.
- [x] Revisions/guidance/consensus remain downstream expectation-gap inputs and are explicitly excluded from AO-K0A's Q/M-perp denominator/orthogonalization preflight.
- [ ] Only one frozen AO-K0B survivor/version may approach W6 after lawful full-W3 development survival; W6 failure closes that version with no rescue.
