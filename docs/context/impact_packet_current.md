...
New governing deltas: CRV1 stays 252d but is no longer the sole programme clock; one fast multi-week family is authorized for preregistration; default active family WIP=`2`, ceiling=`3`; evidence qualification may happen in parallel while current portfolio/capital-policy commits remain singular; PAPER-0 is authorized behind minimum identity/TIF/broker-state/restart/fencing/calendar gates; replication readiness starts now in quarantine; historical A1/A2 is hard-blocked until filing-vintage + same-input AOV parity close.
