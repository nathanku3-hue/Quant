...
- `PM_DELTA`: `Feature work can resume only for research-only lanes; live execution, recommendations, ranking, scoring, action alerts, and replay-output certification remain blocked.`
- `GOVERNANCE_DELTA`: `GovernanceGateV0, runtime boot-status path contract, data readiness, context validation, portfolio AppTest smoke, focused replay/dashboard contract, and GOV-009 execution inventory are PASS on the target baseline.`
- `RUNTIME_DELTA`: `runtime/boot_status_current.json is generated by strict preflight only, ignored, untracked, and local runtime evidence rather than a committed artifact.`
- `OPEN_DECISION`: `Choose the next research-only feature lane, preferably rendered dataframe/AppTest governance expansion for safety or one G9 market-behavior signal card for product momentum.`
- `RECOMMENDED_NEXT_STEP`: `review_merge_feature_development_reopen_anchor_then_start_research_only_feature_branch.`
- `DO_NOT_REDECIDE`: `Do not treat safe_boot=true as authorization for live trading, broker automation, recommendations, ranking, scoring, action alerts, provider refresh during boot, or replay-output certification.`
