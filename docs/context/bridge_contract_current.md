...
- RECOMMENDED_NEXT_STEP: bank local C1 candidate on exact base `b7a24d3` via explicit b7→donor-byte transformation (18 authored/input paths + 2 regenerated current-context outputs + 1 tracked candidate manifest = 21 paths), prove clean tree, then push and run hosted/independent reviews. Do not open G08 session in the banking round. Publish every valid later result: `IMPROVED` merges as S-009X PASS; `NOT_IMPROVED` is retained falsification with no uplift and triggers replan.
