...
## AO-K0A Denominator Alignment / Orthogonal Basis sentinels — current

- Universal stock-discovery scientific unit=`OpportunityKernelV1`; historical `ECONPHYSICS_PREBREAKOUT_v1` remains immutable and specialized. Active basis authority=`OrthogonalizationContractV1`; this freeze has **no empirical Alpha result**.
- Denominator=`PREBREAKOUT_US_PRIMARY_COMMON_DATE_LOCAL_V1` exactly. Missingness is persistent state, never eligibility rewrite or coverage PASS/FAIL. Statuses=`ELIGIBLE_COMPLETE / Q_UNOBSERVED / M_WARMUP / M_MISSING_HISTORY / Q_AND_M_MISSING`; ABSTENTION=`selected=false / risky_weight=0 / no denominator relief`.
- Q rank denominator=all Q-observed W3. For residual arms, Q and M are both re-ranked on Q∩M before date-local OLS; `M_perp` is the residual. Q arm is not forced to the joint intersection.
- Full-W3 evaluation sentinel: right-tail denominator=all matured W3 winners; breadth denominator=all W3 names; abstain winners=foregone upside; abstain bottom-tail=avoided catastrophe. Complete-case denominator, security/peer return imputation and observed-subset renormalization are forbidden.
- Capital sentinel: residual capital=canonical economic cash; opportunity comparator=PIT equal-weight full W3. These are separate semantics and neither imputes a missing security return.
- Source-boundary sentinel: only immutable W3 + admitted S0 + exact W3 market custody may define observability. Test assemblies, transient feature stores and alternate-listing bridges are forbidden. Source preflight=`310329` rows, matrix SHA-256 `bd36a6305f38ff68c57f6ccfb9d3481be6fd42d2288128ef9ec3eb3cc12df5cf`, zero rows deleted, W6/outcome/provider reads=`0`.
- Historical ~79.49% common coverage is not recertified and is not authority. Numeric Q is not rederived from legacy artifacts; scalar Q+M-perp composition remains unbound until AO-K0B pre-result source freeze.
- Trial charge sentinel: AO-K0A freeze=`0`; first exact result-bearing `Q / M_perp / Q+M_perp` development run=`1` material basis trial, charged only after numeric Q/composition law, exact inputs and uninspected labels are hash-bound.
- Hard stop sentinel: no empirical basis read, W6, provider, K tuning, dislocation experiment, peer valuation, portfolio optimization, A2 re-query, Parent/Child/Clock #1 mutation, broker or capital escalation. `financial_alpha_evidence=0`.

## PREBREAKOUT Trial #1 / Econphysics sentinels — current

- Historical W2 Trial-1 authority remains exactly `PREBREAKOUT_W2_CONTRACT_v1`; B/B-1/TTFLD and 20d/10d top-5% label definitions are immutable evidence for Trial #1. AO-K0A is the explicit later denominator/orthogonalization authority for the universal basis and binds directly to W3; W2 bytes are not silently inherited as AO-K0A acceptance authority.
...
- `ECONPHYSICS_PREBREAKOUT_v1` causal contract + PIT observable manifest remain frozen, and a real admitted S0 structured corpus now exists locally. The frozen M0/M1 persistence semantics both fail, but the fixed low-freedom dynamics diagnostic retains inventory-channel and margin-state information under mean-reverting operators; revenue has no stable survivor. Therefore observable insufficiency is **node-specific and not globally established**. If new upstream data are needed, prioritize orders/backlog, pricing/mix, utilization/capacity and channel inventory. Revisions/guidance/consensus remain a separate downstream expectation layer; market confirmation remains downstream-only. CRV1 causal vocabulary/Alpha-PIT row semantics may be reused, but CRV1 252d outcome/risk-set/search/prediction authority may not.
- The former universal successor-M0 framing is superseded by AO-K0A. Inventory/margin dynamics remain diagnostic candidate information only; the active endgame basis seam is the full-W3 Q/M-perp orthogonal contract above.
- **Today authority-escalation stop sentinel:** no provider capture for AO-K0A, no basis empirical trial/read, no successor prediction clock, no W6. W6 remains `lockbox_opened=false / labels_opened=false`. Existing diagnostic bytes remain readable for governance/history only and may not mutate the frozen AO-K0A basis.
...
- Current-cut AOV CIQ bytes may be consumed by the Alpha PIT structured adapter only at/after their conservative availability boundary; they must never be backdated into historical PIT.
- AOV frozen-109 growth-screen membership must never become `CRV1_US_PRIMARY_COMMON_V1`. A CRV1 risk-set source requires the frozen eligibility-contract hash, no growth/current-survivor/future-membership filter, row-level U.S. primary-common/primary-listing/active-tradable/unique-identity/200-observation proof, and a separately hash-bound identity receipt.
...
- PAPER-0 implementation is authorized; first order is not authorized until `ExecutionIntentV1`-equivalent authority binds account/rebalance/policy-seal/map/instrument/side-qty/execution-policy/TIF/epoch and `market+cls` reaches the broker payload.
- Broker lifecycle must project into a canonical PAPER state with open orders/partial residuals committed; restart starts `FREEZE_NEW_RISK=true` and only clean reconciliation clears it.
- First PAPER order must resolve actual session close or be restricted to a verified regular full-session day and fail closed otherwise.
- Rebalance epoch/fencing prevents stale workers from creating new risk.
- Replication-readiness entitlement/identity/PIT/license work starts now in quarantine; replication outcomes remain inaccessible to discovery/confirmatory research.
- Historical CIQ bounded concurrency begins only after the vintage semantic is frozen; start at two independent Excel workers and scale only from measured provider/COM stability.
...
Stop on compatibility reintroduction, mutable evidence identity, unnamed benchmarks, non-finite cost acceptance, unadmitted current data, in-place insurance-V0 changes, substituted official cash source, early outcome opening, misleading historical-vintage labels, A1 without current/historical parity, family cross-contamination, PAPER without TIF/state/restart/calendar closure, or alpha/live claims before mature replicated evidence.
