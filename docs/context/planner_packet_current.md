...
→ LANE 2 COMPRESSED LEARNING: reconstruct exact historical start risk set + historical primary identity BEFORE A1/A2
...
2. Finish historical Company Type / Company Status authority for the frozen 104-name 2025-05-16 market+Original-revenue candidate cohort; do not substitute current profile labels or the current frozen 109.
3. Finish historical-effective provider-primary Security/Trading Item authority for the exact final cohort; the unique dated row + 104/104 current-primary corroboration is evidence, not the effective-dated primary receipt.
4. Admit/hash-bind the final historical risk set + historical-primary receipts, then materialize only that admitted cohort's historical market/fundamental bytes under frozen `FilingVer=Original`; use bounded Excel workers only within measured stability.
5. Run A1; if admitted, freeze and run A2 exactly once under the existing hidden-query law, then put Parent/Child compounding, drawdown, turnover/cost/cash drag, loss concentration, missed winners, under-sizing and winner clipping on the board.
