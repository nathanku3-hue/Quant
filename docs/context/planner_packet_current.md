...
- `OrthogonalizationContractV1` is frozen at `docs/architecture/orthogonalization_contract_v1.md`; machine freeze=`docs/architecture/ao_k0a_orthogonal_basis_preflight_v1.json`. The older AO-K0 constitution remains historical evidence but is superseded where it used a separate AO denominator, raw `Q×M`, or coverage-failure gates.
- Active denominator=`PREBREAKOUT_US_PRIMARY_COMMON_DATE_LOCAL_V1` exactly. Missingness is a persistent full-W3 state (`Q_UNOBSERVED / M_WARMUP / M_MISSING_HISTORY / Q_AND_M_MISSING`), never eligibility rewrite and never `PASS/FAIL`. ABSTENTION means `selected=false`, `risky_weight=0`, no denominator relief.
- Q rank is date-local over all Q-observed W3 names. M residualization re-ranks Q and M on the exact Q∩M sample, then fits `rank(M)=a_t+b_t*rank(Q)+epsilon`; `M_perp=epsilon`. No temporal fitting or outcome input. Q arm retains its own observability advantage; M-perp and Q+M-perp require joint observability.
...
- The real admitted S0 corpus is now in local custody: `23,047` snapshots / `5,382` securities / `17,662` predecessor-gated adjacent transition pairs. The frozen M0/M1 v2 shootout remains a valid failure of the original **persistence** semantics; it is not evidence that structured fundamentals contain no transition information.
- A diagnostic-only fixed low-freedom operator family was run on the same PIT corpus/folds/XS holdout with no market, winner, W6, K, threshold tuning, bootstrap or provider access. Inventory survives via `DELTA_MEAN_REVERSION`, `M0_STATE_MEAN_REVERSION`, and `M1_STATE_MEAN_REVERSION`; operating margin survives via `M1_STATE_MEAN_REVERSION`; revenue has no `>=3/4` survivor. Integrated routing=`NODE_SPECIFIC_DYNAMICS_SURVIVORS`; global `OBSERVABLE_INSUFFICIENCY` is **not established**.
- Inventory direct lag-1 reversal by fold=`61.305% / 54.930% / 57.738% / 64.706%`; reversed M0 lift=`1.038 / 1.038 / 1.104 / 1.121`, reversed M1 lift=`1.007 / 1.049 / 1.118 / 1.032`. Margin reversal=`61.888% / 54.679% / 56.976% / 64.965%`; reversed M1 lift=`0.986 / 1.068 / 1.185 / 1.204`, so folds 2–4 satisfy the existing lift/association gate.
- This is posterior diagnosis, not a fresh preregistered confirmation or Alpha result. Retain inventory/margin for a separately frozen causal continuation; revenue may route toward new upstream causal observables. Keep orders/backlog, pricing/mix, utilization/capacity and channel inventory upstream; keep revisions/guidance/consensus in the downstream expectation layer. Evidence=`docs/context/e2e_evidence/econphysics_prebreakout_s0_dynamics_diagnostic_20260811.json`; `financial_alpha_evidence=0`; W6 remains untouched.
...
- Successor identity is `ECONPHYSICS_PREBREAKOUT_v1`, not old-family Trial #2. The 2026-08-10 all-or-nothing Stage-1 timeout remains immutable historical evidence, but it is superseded as current execution truth by the later restartable capture now in local custody (`structured_transitions.csv` receipt transport=`RESTARTABLE_ATOMIC_PROVIDER_BATCH_SHARDS_V1`). Current scientific state is the real-corpus M0/M1 persistence FAIL plus the 2026-08-11 dynamics diagnostic: inventory and margin retain node-specific transition signal under low-freedom mean-reversion operators; revenue does not. This creates no winner-selection, prediction-clock, W6, or capital authority and keeps `financial_alpha_evidence=0`.
