...
ACTIVE_STATUS: `WINNER/AUTHORITY RECUT APPROVED ~99/100; TOP-LEVEL DESIGN EFFECTIVELY CLOSED; PRE_SEAL_REAL_CIQ_ADMISSION UNCHANGED; CIQ IDENTITY + PRIMARY MASTER RAW CUSTODY SOLVED; REAL CLOCK BLOCKED ON COMPLETING >=200-DAY PRIMARY-SPT MARKET CUSTODY + ADMISSION`
...
- Real `official_sofr.parquet` now exists at the seal input path. The 109-name primary Security/Trading Item master is also now a real timestamped/hash-bound raw provider object, but admission is still pending. Market custody remains incomplete: nine earlier 5-day parts plus a validated exact-SPT 7-day target-week part exist, while the full >=200-completed-observation series is unfinished. Therefore real current AOV Rule100 targets/primitives/returns/decision-cut still do not exist.
- No A1 real AOV result, no real prospective seal, no A3 evidence.
- Latest Winner/authority recut is owner-approved at ~99/100 after recuts. Top-level design is effectively closed: `CYCLE_RESONANCE_v1` is preregistered as the first Alpha Discovery Lane but `NOT IMPLEMENTED`; the Right-Tail Atlas is Minimum-Viable-Atlas-first rather than full-history-first; outcome-visible right-tail reverse engineering is a discovery-only **episode census** with false-winner, missed-right-tail and matched ordinary/left-tail contrasts; Rule100/Parent/Child winner-blindness is diagnostic only; discovery/entry, continuation/hold and exit/falsifier alpha remain separable. Future current-authority transitions obey the system-wide Destructive Authority Replacement Law; owner governance is six constitutions; and implementation forks choose one incumbent rather than parallel menus. None of this changes `PRE_SEAL_REAL_CIQ_ADMISSION` or `financial_alpha_evidence=0`.
...
C. real CIQ admission                                       [OPEN MARKET CUSTODY]
   109-name primary Security/Trading Item master [CAPTURED RAW / NOT ADMITTED]
   + complete >=200 daily SP_TOTAL_RETURN/SP_PRICE_CLOSE/SP_VOLUME rows
     using exact primary SPT keys in 7-weekday atomic chunks
   -> combine/hash raw market object -> builder -> rule100_targets + vertical_primitives + total_returns + decision_cut_v3
   (identity/provider capability solved; official_sofr.parquet already admitted)
...
Architecture is closed. Active P0 is now **real CIQ raw custody/admission only**. Provider capability and the required market field semantics are already proven. No refactor, optimizer, performance work, historical-PIT rebuild, legacy SNLQuery/SNLPrice retry, or platform work may enter the first Clock-Start critical path.

## Immediate sequence

1. Reuse and verify the captured 109-name primary master `data/aov0/raw/ciq_primary_security_master_20260808T162322Z.csv` (SHA-256 `8aefbbd751a714b8689402ccbf8fa2b776c6388d4bbc3870ec4f8b975306eca4`). Do not redo Identifier Lookup traversal unless auditing the mapping; do not substitute ticker/company identity.
2. Complete the remaining historical market custody with `tmp/ciq_capture_market_chunk.ps1 -UseTradingItemKey`, exact primary SPT keys, and 7 weekdays per atomic query. 7 weekdays is the largest proven stable width; 8/10-day widths failed. Existing custody includes nine earlier 5-day parts plus validated target-week exact-SPT `part_033_20260730_20260807.csv` (SHA-256 `81779d8ed04b80a5b79298821f9ff5d89c96abf025e8bb06d9a67f864cd744aa`).
3. Combine only real provider part CSVs, dedupe on (`SPT_DATE`,`SP_CIQ_ID`,`SP_TRADING_ITEM_ID`), retain exact field/source/retrieval metadata, hash the combined raw market object, and verify actual completed observations per security. Requested weekdays are only a cushion; the hard gate is >=200 completed closes through `2026-08-07` after the builder's existing factor-coverage filter. Do not backfill short-history names from alternate listings.
4. Restore the repo-approved `.venv` interpreter before Python execution; the `.venv` directory exists but `.venv/Scripts/python.exe` is absent at handover, and system Python must not be substituted. Then run the existing fail-closed CIQ builder → `decision_cut_v3` → real Seal Candidate → fresh-process full-chain verification → immutable Clock-Start Receipt. Only the receipt starts the clock.
...
7. After Clock #1, enforce one active **Alpha build** lane, not one running experiment. Release the preregistered `CYCLE_RESONANCE_v1` lane first: build only enough legitimate PIT history for honest discovery/contemporaneous controls → freeze the family/implementation/falsifiers/search budget → untouched PIT test where available → seal prospectively as soon as honest. Deeper historical work underneath the running tape may enumerate all qualifying covered right-tail episodes and reverse-engineer logic, but it remains discovery-only and must retain false-winner, missed-right-tail, matched-control, incumbent winner-blindness, and entry/continuation/exit decompositions. Weekly AOV tape + review/custody remain active.
