...
- Active product gate: `PRE_SEAL_REAL_CIQ_ADMISSION` → first real immutable AOV-0 five-arm prospective clock-start receipt.
...
- Roadmap/change authority is `docs/architecture/aov_endgame_generalization_spec_current.md`. Prior architecture/scientific-bar, strategic-reorder and sim-to-real approvals remain valid on their scopes. Latest owner alignment is `~99/100` after recuts with verdict `APPROVED_WITH_WINNER_AND_AUTHORITY_RECUT`; top-level design is effectively closed. Current execution remains `PRE_SEAL_REAL_CIQ_ADMISSION`: v3 temporal/adversarial authority is locally green and real CIQ admission remains open. `CYCLE_RESONANCE_v1` is preregistered as the first post-Clock Alpha Discovery Lane but is `NOT IMPLEMENTED`; no Winner/Atlas/AI/broker work is authorized before Clock #1.
...
- After Clock #1, roadmap topology remains parallel clocks: weekly AOV evidence never stops; one active build lane can seal multiple running tapes; independent replication/borrow clocks start when dependencies permit; `CYCLE_RESONANCE_v1` is released first; and independent Ops/Engineering starts the thin Alpaca PAPER Capitalization Vertical. The Alpha lane follows Minimum-Viable-Atlas-first: enough legitimate PIT history for honest discovery/controls → freeze one family → untouched test where available → seal prospectively as soon as honest, while deeper risk-set/false-winner/replication work continues underneath. The PAPER path uses one incumbent (`MOC_CLOSE_AUCTION_V1 = market + cls`), one promoted policy/seal→`live_rebalance_id`, PIT `CIQSEC`↔broker account/instrument mapping, full lifecycle/open-order authority, broker-first restart reconciliation/fencing/`FREEZE_NEW_RISK`, and dual research↔broker P&L. Alternatives are challengers, not parallel architecture. System-wide Destructive Authority Replacement removes old active readers/writers/fallbacks/aliases when new authority lands; old artifacts remain historical evidence only. Shared abstraction follows the Rule of Two; simple allocation remains incumbent; optimizer remains challenger; bounded long/cash does **not** wait for true L/S.
