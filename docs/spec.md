...
- Roadmap/change authority is `docs/architecture/aov_endgame_generalization_spec_current.md`; final strategic lock is `docs/architecture/aov_strategic_direction_lock_20260809.md`. Final re-audit=`PASS — STRATEGIC_DIRECTION_LOCKED`; broad architecture reopen=`NO`; mandatory recuts are authorized execution gates. CRV1 remains the unchanged 252d slow business-cycle family; one fast multi-week family is authorized for preregistration. Default active family WIP=`2`, initial ceiling=`3`; parallel evidence qualification is allowed, while the current portfolio/capital-policy commit chain remains singular. Post-Clock objective=`EVIDENCE_VELOCITY × ECONOMIC_RELEVANCE`.
...
- AOV-0 candidate-universe membership remains the frozen original 109 entities. Weekly seals refresh current fundamentals, primary-security/trading status, completed market data and rate/benchmark state for those same entities; the high-growth screen is not rerun weekly. REQUIRED stale/missing measurements fail closed according to their data contract.
- After Clock #1, topology follows **parallelize prediction clocks/evidence qualification; serialize current portfolio/capital-policy commits**. **Future Truth:** weekly frozen-109 prospective evidence remains always-on; CRV1 keeps its 252d primary horizon; one separately preregistered fast multi-week family may run beside it, with default family WIP=`2` and ceiling=`3`. **Historical Compression:** historical PIT CIQ → exact frozen-AOV A1 → frozen query-metered A2 remains the target. Historical fundamental vintage is frozen to `CIQ SPG historical as-of + FilingVer=Original`, and current↔historical replay parity is closed by routing historical decisions through the exact current-cut builder with next-session cube activation. A1/A2 is now hard-blocked on the exact historical high-growth start risk set plus exact historical provider-primary Security/Trading Item identity for that same cohort/date. **PAPER-0:** smallest lineage-correct `ExecutionIntentV1` → `market+cls` → broker canonical state → restart/fencing/`FREEZE_NEW_RISK` path is authorized; first order must resolve actual session close or fail closed to a verified regular full-session day. **Replication readiness:** entitlement/identity/PIT/license quarantine starts now. Bounded AI may support multiple family clocks but outcome-informed mutation still requires a matured validated ReviewPacket. Market Transition remains discovery-only unless explicitly admitted into family WIP; leverage/short/options remain evidence/CRO-gated. One writer per mutable authority surface, deterministic immutable joins, Destructive Authority Replacement and Rule of Two remain mandatory.
...
- Mechanical/test seals and Clock #1 startup are not financial-alpha evidence. Portfolio-alpha evidence remains `0`; prospective clock is running but pre-evaluation/outcome-sealed. A1/A2 are historical evidence levels rather than prospective financial-alpha proof and are currently blocked on historical start-risk-set membership + historical primary Security/Trading Item authority; vintage/parity are closed. PAPER-0 is operational evidence only. Limited Live remains closed.
