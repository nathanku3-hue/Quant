...
- Production insurance V0 is frozen at `insurance_materiality_floor_ratio=0.05` and `insurance_premium_ceiling_annual_return=0.0015`; changing either creates a new contract/model family and does not calibrate V0 in place.
- Active equity authority is `AOV0-CIQ-AUTHORITY-V1`: `CIQSEC:<Capital IQ Security ID>` is the sole AOV security identity, and S&P Capital IQ Pro owns the single `run_4` company-universe/fundamentals object, primary-security master, and primary-security total returns. The prior CRSP/PERMNO route and separate `run_2` screen dependency are superseded for active AOV; no ticker, company `SP_ENTITY_ID`, legacy PERMNO, or dual-provider compatibility is allowed.
- The four prior prospective-custody findings and the later v3 temporal/adversarial gate are implemented locally and are no longer active blockers. Current authority is destructive v3: `aov0_ciq_decision_cut_v3`, `aov0_prospective_seal_v3`, and `aov0_prospective_clock_start_receipt_v1`; `execution_calendar_id=NYSE_2026_CORE_CLOSE_1600_ET`; active v2/open readers and writers are absent. Seal construction produces a non-authoritative Seal Candidate; a fresh child process must produce full-chain verification proof, and only a separate immutable Clock-Start Receipt may start the clock. Execution/evaluation time, CIQ daily-return interval, and 30-day maturity are one economic contract: next eligible close, attribute only intervals beginning at/after evaluation start, maturity = evaluation_start + 30 calendar days. Adversarial authority tests pass locally. Real Rule100 targets, vertical primitives and PIT total returns remain required; direct NY Fed SOFR is already admitted.
- `run_4.xlsx` is the single hash-bound frozen 109-company universe + bounded current-cut company-fundamental source: 1,203 absolute `FQqYYYY` entity-quarter rows produce 109 current factor states with canonical Rule100 V1.1 `factor_positive_count`; relative `FQ0` is excluded, one entity has no absolute-quarter history, and incomplete historical publication timestamps force conservative local-admission `known_at`. Its receipt carries both `COMPANY_UNIVERSE` and `QUARTERLY_FUNDAMENTALS`. `run_2.xlsx` is historical evidence only. `run_4` is not historical PIT replay proof or permanent security authority: `SP_ENTITY_ID` remains temporary, and Capital IQ Security ID, `technical_quality`, final Rule100 targets, primitives, and total returns still require the primary-security master/market leg.
- Economic cash authority is official SOFR minus 25 bp, ACT/360, no zero floor, post-publication only, no proxy substitution. Arm 5 is therefore `ECONOMIC_CASH`; PIT equal-weight eligible universe is the risky-asset performance comparator.
- ZERO-COMPAT seven-count scan must remain all zero, including no archived/release executable-source import outside receipt-integrity tests.
- Roadmap/change authority is `docs/architecture/aov_endgame_generalization_spec_current.md`; final strategic lock is `docs/architecture/aov_strategic_direction_lock_20260809.md`. Final re-audit=`PASS — STRATEGIC_DIRECTION_LOCKED`; broad architecture reopen=`NO`; mandatory local recuts remain execution gates. CRV1 stays the unchanged 252d slow business-cycle family; VSB is parked as `VSB_CONFIRMATION_v1` only; ETF-first Sector Rotation remains orthogonal; `ECONPHYSICS_PREBREAKOUT_v1` is the new stock-winner discovery methodology identity. Predecessor Trial #1 is a valid scientific FAIL permanently charged `1/8`; W3 is complete, W4 sealed/verified, W5 failed, W6 untouched. The successor causal/PIT methodology is frozen but has no capture/empirical-trial authority today. Parallel evidence qualification remains allowed while current portfolio/capital-policy authority stays singular. Post-Clock objective=`EVIDENCE_VELOCITY × ECONOMIC_RELEVANCE`.
- **Result-first research method (2026-08-12):** `docs/architecture/result_first_ai_research_loop_v1.md` is the forward-only AI/scientific-method lock; machine preflight=`docs/architecture/opportunity_kernel_scientific_preflight_v2.json`; action-bearing feasibility=`docs/architecture/execution_feasibility_v1.json`. Material slices preregister the decision and PASS/FAIL/UNRESOLVED routes; identical routes mean `DO_NOT_RUN`. L4 means `READY_FOR_L5_CANDIDATE`, never automatic L5; scarce L5 is allocated across families with ordinal RDV (`LOW/MED/HIGH`) and may return `READY_BUT_NOT_PRIORITY`. Hard risk stays a veto, risk margins stay a vector, execution telemetry calibration is policy-conditioned, and fills never become universal Alpha features. The reuse default is two real consumers, with an earlier exception only for safety/authority primitives where duplication itself creates risk. Historical evidence and current product/capital authority are unchanged.
- **AI-native constitution vs runtime (2026-08-12):** architecture is constitutionally YES; runtime is PARTIAL. Status machine=`docs/architecture/ai_native_runtime_status_v1.json`; closure audit=`docs/architecture/ai_native_runtime_closure_audit_20260812.md`. Universal preflight enforcement, cross-family RDV allocator, and universal outcome-authority gate remain NOT_IMPLEMENTED. Top residual GQs: constitutional enforcement closure (legacy outcome bypass), risk-state provenance (`UNRESOLVED` for missing hard-risk inputs), endogenous observability. Scientific L0–L11/P1–P4 namespace stays authoritative; SCIENCE_S5 rename rejected. Authority cut=`codex/pit-source-authority-1` worktree; public main non-authority until merge.
- Right-Tail Atlas forensics are `DISCOVERY_ONLY`: under the frozen family outcome, enumerate all qualifying episodes inside the legitimately covered date-local risk set rather than famous winner companies; retain true-right-tail, false-winner, missed-right-tail, and matched ordinary/left-tail contrasts; audit Rule100/Parent/Child winner-blindness without tuning; and treat discovery/entry, continuation/hold, and exit/falsifier skill as separable Alpha Components/Families. Outcome-visible forensic patterns generate hypotheses/search debt only and never count as untouched/OOS/prospective evidence.
- AOV-0 candidate-universe membership remains the frozen original 109 entities, bound in W1 to semantic universe hash `f4f7ac7ed1ff21b95580bb623953e875bda0106ed478887b7d5ed6ccff456075` and source `run_4.xlsx` SHA-256 `17f356e47c9e3ecf9600ad21db153338f4941272e0a1ffb9fd7b872c6636f056`. W1 current truth is `WAITING_FOR_LEGITIMATE_FRESH_WEEKLY_SOURCE_RECEIPTS`: its current engineering slice is closed, the custody lane remains recurring/event-driven rather than continuously staffed, and no fresh provider acquisition is authorized under the 2026-08-10 methodology hold. When a legitimate weekly custody event is authorized, refresh current fundamentals, primary-security/trading status, completed market data and rate/benchmark state for those same entities; the high-growth screen is not rerun weekly. REQUIRED stale/missing measurements fail closed according to their data contract; no additional W1 custody code is added absent a concrete defect.
- After Clock #1, topology follows **parallelize evidence qualification; serialize current portfolio/capital-policy commits**. Weekly frozen-109 custody remains event-driven; CRV1 keeps 252d; ETF-first Sector Rotation stays independent; VSB stays confirmation-only. Historical Compression A1/A2 is **closed** after one frozen A2 read and must not be re-queried. PREBREAKOUT Trial #1 is closed FAIL at permanent `1/8`; successor `ECONPHYSICS_PREBREAKOUT_v1` is method-frozen with W6 untouched. Its next lawful future sequence is demand-pulled PIT capture → economic-state validation with winner labels sealed → ex-ante selection-budget/K freeze → full-census winner evaluation → ~4-fold OOS + deterministic cross-sectional holdout → freeze one survivor → W6 once; **none of these empirical steps is authorized today**. PAPER-0 and replication readiness remain independent. Bounded AI may support research only under existing role/visibility firewalls; outcome-informed mutation still requires a matured validated ReviewPacket. Market Transition stays discovery-only; leverage/short/options remain evidence/CRO-gated. One writer per mutable authority surface, deterministic immutable joins, Destructive Authority Replacement and Rule of Two remain mandatory.
- AI/external systems may generate source candidates, hypotheses, controls, explanations and bounded mutation drafts only; deterministic admission, Trial/Prediction custody, prospective evidence, replication and risk governance own financial authority. `AIInvocationReceipt` and role/visibility firewalls are the required future provenance boundary; no generic agent platform is authorized.
- Destructive shipping applies to obsolete **current runtime authority**: when new authority wins, old active reader/writer/fallback/alias/dual-write/old-authority feature flag/compatibility adapter are removed in the same slice. Immutable historical evidence remains readable only under its pinned historical environment/schema. PIT timing, receipts/hashes, prospective seals, Trial/Search custody, exact reconciliation and hidden-OOS discipline are never weakened in the name of speed.
- Owned Alpha/Lane-2 deterministic gates may ship without waiting for unrelated inherited repository-wide failures; those failures still block a repository phase-close claim and must remain explicit.
- Mechanical/test seals and Clock #1 startup are not financial-alpha evidence. Portfolio-alpha evidence remains `0`; prospective Clock #1 remains outcome-sealed. Lane2 A1/A2 is closed historical diagnostic evidence, not prospective proof. PREBREAKOUT Trial #1 is also historical/development scientific evidence only; its failure creates no financial-alpha or capital authority, and the econphysics successor remains pre-production methodology-only today. PAPER-0 is operational evidence only. Limited Live remains closed.
