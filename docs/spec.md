...
- Roadmap/change authority is `docs/architecture/aov_endgame_generalization_spec_current.md`. Prior architecture/scientific-bar, strategic-reorder, sim-to-real and Winner/authority approvals remain valid. Latest verdict remains `APPROVED_WITH_VELOCITY_PARALLELISM_RECUT`, with owner execution recut=`A1_A2_SECOND_CRITICAL_LANE` and owner rating=`9.77/10`. Top-level design is effectively closed. Real CIQ admission, v3 Seal Candidate, fresh-process verification and Clock #1 are complete. Post-Clock objective=`EVIDENCE_VELOCITY × ECONOMIC_RELEVANCE`.
...
- After Clock #1, roadmap topology follows **parallelize work / serialize authority** with two first-class critical evidence lanes. **Lane 1 — Future Truth:** weekly frozen-109 prospective evidence + deterministic review stay alive while one narrow `alpha_pit_data_api_v1` producer and the first confirmatory Alpha-family build (`CYCLE_RESONANCE_v1`) advance behind frozen interfaces; real outcome-informed mutation still requires a matured validated ReviewPacket. **Lane 2 — Compressed Learning:** historical PIT CIQ reconstruction → exact frozen-AOV replay → A1 → freeze immutable A2 split/query/metric/executable contract → query-metered untouched historical PIT OOS → A2 → Parent/Child incremental economics + incumbent loss/missed-winner diagnosis. Lane 2 may not open Clock #1 outcomes or tune Parent/Child between A1 and A2; if historical universe/source/availability authority is insufficient, the output remains an explicit diagnostic rather than A1/A2 evidence. A1/A2 may inform `CONTINUE / PIVOT / HOLD` for the challenger programme, but material changes to an already-frozen CRV1 object require a new version/family rather than in-place rescue. Bounded AI research tooling may build only with independent ownership; Market Transition remains discovery-only until the Alpha-family slot is available; `RESONANCE_LEVERAGE_POLICY_v1` remains evidence/CRO-gated with no current leverage/short/options authority. PAPER Capitalization remains independently parallel. One writer per authority domain, deterministic immutable join gates, Destructive Authority Replacement, Rule of Two and one-incumbent policy remain mandatory.
...
- Mechanical/test seals and Clock #1 startup are not financial-alpha evidence. Portfolio-alpha evidence remains `0`; prospective clock is running but pre-evaluation/outcome-sealed; A1/A2 are historical evidence levels rather than prospective financial-alpha proof; Limited Live remains closed.
