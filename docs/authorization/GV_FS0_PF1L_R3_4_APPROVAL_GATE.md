# GV-FS0-PF1L-R3.4 — Lineage-Correct Protocol Re-Freeze (Self-Contained Approval Gate)

```text
Mode:                 APPROVAL_GATE
GateID:               GV-FS0-PF1L-R3.4
RoundID:              ROUND-PF1L-R3-4-APPROVAL
ScopeID:              GV_FS0_PROTOCOL_LINEAGE_REFREEZE_R3_4
...
5. Payload bytes must contain **no** C0 control characters other than HT (U+0009) and LF (U+000A). Form feed (U+000C) and CR (U+000D) are forbidden.
...
R3.3 is **not** an execution authority target. **R3.4 is self-contained** and supersedes R3.3.
...
| F CI | Checkout **exact F full SHA**; frozen empty-diff vs C; sections 6-7 marker/field/binding checks |
...
Each matched closure report under section 7 globs is Markdown: **exactly one** section 6.2 marker line equal to `candidate_commit_c`.
...
7. F: binding JSON duplicate-key-clean; section 6 Markdown markers + `current_context.json` structured `candidate_commit_c` pass; frozen empty-diff; Thin SAW PASS.
8. G binds F without requiring `closure_commit_f` inside F.
9. Final tip: C,F in Ancestors(tip); no rewrite.
