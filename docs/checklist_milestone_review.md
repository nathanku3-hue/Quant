...
- [ ] **Git Sync**: Are all tracked evidence artifacts committed and pushed to origin/main? (No modified tracked files: `git diff --exit-code HEAD` passes AND no unpushed commits: `git log origin/main..HEAD --oneline` empty. Untracked files may remain intentionally parked.)
