...
def _load_oneshot_authorization(path: Path) -> dict[str, Any]:
    data = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(data, dict):
        raise SystemExit("oneshot authorization must be a JSON object")
    return data
...
def _preregistration_sha256() -> str:
    ...
    if not path.is_file():
        raise GvE0bDv1Error("E0B_PREREGISTRATION_MISSING")
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_session.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to remotely retained one-shot Attempt-1 authorization JSON",
    )
    ...
    p_rub.add_argument(
        ...
        type=Path,
        required=False,
        help="Fixture/local authoring only; real reviewer must use external receipt",
    )
    p_rub.add_argument(
        "--external-receipt",
        type=Path,
        required=False,
        help="GitHub receipt JSON for operationally separated reviewer submission",
    )
    p_rub.add_argument(
        "--external-rubric",
        type=Path,
        required=False,
        help="Exact external rubric bytes path matching the receipt hash",
    )
    ...
    try:
        ...
        if args.cmd == "open-session":
            ...
            prereg_sha = _preregistration_sha256()
            oneshot = _load_oneshot_authorization(args.oneshot_authorization)
            verify_oneshot_authorization(
                oneshot,
        ...
        if args.cmd == "submit-rubric":
            ...
            if args.external_receipt is not None or args.external_rubric is not None:
                if args.external_receipt is None or args.external_rubric is None:
                    raise GvE0bDv1Error("E0B_EXTERNAL_RECEIPT_MISSING")
                external_receipt = _load_authoring(args.external_receipt)
                external_rubric_path = Path(args.external_rubric)
                # Authoring body is taken only from external bytes during seal.
                authoring = _load_authoring(external_rubric_path)
            else:
                if args.authoring is None:
                    raise GvE0bDv1Error("E0B_EXTERNAL_RECEIPT_MISSING")
                authoring = _load_authoring(args.authoring)
            sealed = _checkpointed(
                paths=paths,
            ...
            if sealed.get("external_submission_receipt"):
                print(
                    "receipt_hash="
                    f"{sealed['external_submission_receipt']['receipt_hash']}"
                )
                print("mapping reveal after receipt-bound rubric seal")
            else:
