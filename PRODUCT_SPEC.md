...
## Active Product Specification (2026-08-08)
...
ACTIVE_STATUS = V3_TEMPORAL_AND_ADVERSARIAL_AUTHORITY_PASS_75_OF_75; RUN_4_DUAL_ROLE_AUTHORITY_PLUS_DIRECT_SOFR_ADMITTED; INSURANCE_V0_FROZEN_5PCT_15BP; REAL_CIQ_SECURITY_MARKET_BYTES_REMAIN
...
ZERO_COMPAT = PASS_ALL_SEVEN_COUNTS_ZERO
...
The production AOV-0 V0 contract freezes `insurance_materiality_floor_ratio = 0.05` and `insurance_premium_ceiling_annual_return = 0.0015`. These values require at least 5% relative CVaR/Expected-Shortfall improvement while permitting at most 15 bp/year expected net-return sacrifice; changing either creates a new contract/model family. The active equity contract is now `aov0_ciq_executable_contract_v1`: permanent identity is `CIQSEC:<Capital IQ Security ID>`; one hash-bound `run_4.xlsx` object owns both the frozen 109-company universe and quarterly fundamentals, while primary-security identity and primary-security total returns are admitted only from S&P Capital IQ Pro. The prior CRSP/PERMNO contract and separate `run_2` screen dependency are superseded for active AOV. `SP_ENTITY_ID`, ticker, legacy PERMNO, and dual-provider compatibility are explicitly invalid substitutes. Direct New York Fed SOFR remains unchanged.

`decision_cut.json` is an authority-bearing cut contract, not a timestamp note. Active schema `aov0_ciq_decision_cut_v3` binds SHA-256 for the four Parquets, `contract_hash`, the mechanically recomputed date-local CIQ-security `universe_hash`, `knowledge_cutoff`, `cut_built_at`, `decision_target_date`, `execution_calendar_id=NYSE_2026_CORE_CLOSE_1600_ET`, exact next eligible `evaluation_start`, and four source receipts for `SPCIQPRO:QUARTERLY_FUNDAMENTALS` (the dual-role `run_4` universe/fundamentals object), `SPCIQPRO:PRIMARY_SECURITY_MASTER`, `SPCIQPRO:PRIMARY_SECURITY_MARKET_DATA`, and direct New York Fed SOFR. Every admission receipt carries a retrieval/admission timestamp and raw-object SHA-256; NY Fed SOFR retrieval must be at/after 15:00 America/New_York. The first-seal entrypoint re-hashes inputs before and after experiment execution, independently system-stamps actual `sealed_at`, requires `knowledge_cutoff <= cut_built_at <= sealed_at < evaluation_start`, binds `aov0_executable_byte_manifest_v1`, seals both current target-vector hashes and serialized target vectors, and writes only a non-authoritative `aov0_prospective_seal_v3` Seal Candidate. Maturity is exactly `evaluation_start + 30 calendar days`; attributed daily-return intervals must begin at/after evaluation start. `scripts/aov0_reopen_seal.py` runs in a fresh child Python process and verifies exact executable bytes, seal hash, decision-cut bytes/fields, all four bound Parquets, experiment manifest, every run evidence manifest/file, and recomputed target-vector hashes, then writes a bound fresh-process verification proof. Only a separate immutable `aov0_prospective_clock_start_receipt_v1` may set clock authority. Active v2/open readers/writers are absent. `run_4.xlsx` remains the single frozen 109-company universe + bounded current-cut company-level quarterly-fundamental source: only absolute `FQqYYYY` columns are normalized; relative `FQ0` is excluded; all values are conservatively `known_at` the local raw-object admission time because complete historical publication timestamps are absent. Its receipt carries both `COMPANY_UNIVERSE` and `QUARTERLY_FUNDAMENTALS`. The resulting intermediate panel/current state may derive canonical Rule100 V1.1 `factor_positive_count`, but it is not historical PIT replay proof and is not permanent security authority. `run_2.xlsx` is historical evidence only.

The current-cut Security/market implementation is frozen in `research/aov0/ciq_market.py`: one primary-security master row per admitted entity must mechanically produce `CIQSEC:<Capital IQ Security ID>` plus Trading/Instrument Item ID; ambiguous/missing primary identity and cross-entity collisions are excluded, never repaired. Factor coverage below three groups and market history shorter than 200 observations are excluded. Same-cut market history must provide direct total return or total-return index plus close/adjusted close and volume. `ADV20=rolling20(close*volume)`; `realized_vol=rolling20_std(total_return)*sqrt(252)`; SMA20/SMA200 provide the existing Rule100 technical gate; Rule100 v1 remains on the product/AOV `0.35` max-weight path. On the target date, Q is the bounded monotone transform of current frozen group positivity and U is missing-factor uncertainty; pre-target warmup rows carry neutral Q, maximum U, null factor counts, and no sizing eligibility so current factors are not backcast. L remains date-local ADV20 percentile and R remains date-local slow-trend breadth because those are market-observable history. Historical market rows exist only to warm rolling market/AOV state; since `run_4` is current-cut-only, the admitted target/return artifacts contain one current decision row and do not fabricate historical Rule100 targets. For this U.S.-listed universe, a target on the market-retrieval date requires retrieval at/after 16:00 America/New_York; future-stamped source retrieval times block, and the decision-cut builder independently rechecks the 16:00 completed-daily-bar rule. The decision-cut and first-seal admission paths also require identical target-date Rule100/return/primitive asset sets and exact primitive `total_return` versus P&L return-matrix reconciliation. Direct SOFR is admitted. The full synthetic current-cut path now proves v3 Seal Candidate → fresh-process full-chain verification proof → separate immutable Clock-Start Receipt; the AOV v3/adversarial suite is `75/75 PASS` and ZERO-COMPAT remains seven zeros. No real seal is claimed until real CIQ Security/Trading Item mapping and post-close market bytes/timestamps are admitted; those raw exports are not present in the active repo-local intake paths.

No backward compatibility is required on the AOV path. Historical runtime and CRSP/PERMNO evidence are preserved as immutable/audit receipts; removed root runtimes, ticker/entity/PERMNO aliases, mutable evidence folders, unnamed benchmarks, transitional authority fallbacks, and dual-source active identity are invalid current contracts.
