...
def _write_reconstruction_fixture(tmp_path: Path) -> tuple[Path, Path, dict[str, Path]]:
    ...
    company_audit = tmp_path / "company_state_keydev_audit.csv"
    pd.DataFrame(
        {
            "SP_ENTITY_ID": list(candidate_ids),
            "EventCount": [5, 4, 3],
            "UnresolvedTerminalAtCutoff": ["false"] * 3,
        }
    ...
    company = _write_component_receipt(
        ...
        "company_state",
        {
            ...
            "key_developments_entity_coverage_count": 3,
            "key_developments_provider_row_count": 12,
            "key_developments_response_exception": None,
            "key_developments_audit_csv_name": company_audit.name,
            "key_developments_audit_csv_sha256": _sha256(company_audit),
