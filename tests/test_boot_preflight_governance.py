...
def test_scanner_is_standalone_and_does_not_import_boot_preflight() -> None:
    ...
    policy_doc = "governance_boundary_" + "policy.md"

    assert "scripts.boot_preflight" not in source
    assert boot_import not in test_source
    assert policy_doc not in source
    assert policy_doc not in test_source


def test_artifact_drift_requires_only_standalone_scanner_files(tmp_path: Path) -> None:
    ...
    assert not any(("governance_boundary_" + "policy.md") in finding.message for finding in result.findings)
    assert not any("boot_preflight.py" in finding.message for finding in result.findings)
