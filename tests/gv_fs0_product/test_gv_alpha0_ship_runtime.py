...
def test_launcher_consumes_data_dir_without_leaking_to_streamlit(tmp_path: Path) -> None:
    from launch_alpha import _extract_data_dir

    remaining, selected = _extract_data_dir(
        ["--data-dir", str(tmp_path / "runtime"), "--server.headless", "true"]
    )
    assert selected == tmp_path / "runtime"
    assert remaining == ["--server.headless", "true"]

    with pytest.raises(SystemExit, match="exactly once"):
        _extract_data_dir(["--data-dir", "a", "--data-dir=b"])


def test_launcher_initializes_workspace_and_propagates_clean_child_env(
    ...
    def _call(cmd: list[str], *, cwd: str, env: dict[str, str]) -> int:
        captured["cmd"] = cmd
        captured["cwd"] = cwd
        captured["env"] = env
        return 0

    monkeypatch.setattr(launch_alpha.subprocess, "call", _call)
    result = launch_alpha.main(
        ["--data-dir", str(runtime_root), "--server.headless", "true"]
    )
    ...
    assert env[RUNTIME_HOME_ENV] == str(runtime_root.absolute())
