...
def _has_fail_closed_replay_state(app) -> bool:
...
class TestRealDashboardPolicyTarget:
    ...
    def test_portfolio_route_renders_real_dashboard(self, dashboard_app):
        assert any(header.value == PORTFOLIO_PAGE_TITLE for header in dashboard_app.header)
        assert any(
    ...
    def test_replay_snapshot_rendered_with_ticker_and_replay_weight(self, dashboard_app):
        ...
        assert found_snapshot or _has_fail_closed_replay_state(
            dashboard_app
        ), "Strategy Replay Latest Snapshot dataframe not rendered and replay did not fail closed"
    ...
    def test_replay_snapshot_includes_cash(self, dashboard_app):
        ...
        assert found_cash or _has_fail_closed_replay_state(
            dashboard_app
        ), "CASH not found in any replay dataframe and replay did not fail closed"
